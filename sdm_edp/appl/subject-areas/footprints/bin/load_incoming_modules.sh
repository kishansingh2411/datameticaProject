...
if [[ $table_name == "cmts_interface_mac" ]]
then
    job_name="EDP_FPT_${key_param_id}_CMTS_INT_LD_IN"
else
...
source_file_name=cmts_interface_mac_${formatted_source_date}.txt
source_file_path="${LOCAL_DATA_DIRECTORY}/${source_file_name}"
...
tables=(${INCOMING_CMTS_INTERFACE_MAC_TBL})
for table in "${tables[@]}"
do
   table_name=`basename ${table}`
        
   sh $SUBJECT_AREA_HOME/${table}_tbl/bin/module.sh "transform" "${formatted_source_date}" "${source_file_path}" "${log_file_path}"
   exit_code=$?   

   if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
   then
      fn_log_error "Failed to load data for table "${table_name}" in Incoming layer" "${log_file_path}"
	  fn_update_job_statistics "${batch_id}" "${job_name}" "${start_time}" "0" "${exit_code}"  "${source_date}" "${log_file_path}" "${source_file_name}" " "
   fi 
   fn_log_info "Successfully loaded data for table ${table_name} in Incoming layer" "${log_file_path}"
done
...
record_count=`beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} --showHeader=false --silent=true --outputformat=tsv2 -e \
             "SELECT COUNT(*) FROM $HIVE_DATABASE_NAME_INCOMING.${HIVE_TABLE_PREFIX}${tables} \
              WHERE  P_YYYYMMDD='$formatted_source_date';"`
