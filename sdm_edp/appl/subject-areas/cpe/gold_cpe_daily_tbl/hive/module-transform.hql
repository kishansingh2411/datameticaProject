...
INSERT OVERWRITE TABLE ${hivevar:gold_database}.${hivevar:table_prefix}${hivevar:gold_cpe_daily_table}  PARTITION(P_YYYYMMDD)
SELECT 
...
FROM (
        ...
        FROM (
                    SELECT UPPER(eqmt.REGION_PS_ID)         REGION_PS_ID
                         ...
                         --
                         , eqmt.p_yyyymmdd
                    FROM ${hivevar:gold_database}.${hivevar:table_prefix}${hivevar:gold_equipment_table} eqmt
                         LEFT OUTER JOIN ${hivevar:gold_database}.${hivevar:table_prefix}${hivevar:gold_cpe_ivr_table} ivr  ON (ivr.P_YYYYMMDD  = '${hivevar:source_date}' AND ivr.site_id  = eqmt.site_id AND ivr.acct_nbr  = eqmt.acct_nbr)
                         LEFT OUTER JOIN ${hivevar:gold_database}.${hivevar:table_prefix}${hivevar:gold_cpe_tc_table}  TC   ON (TC.P_YYYYMMDD   = '${hivevar:source_date}' AND TC.site_id   = eqmt.site_id AND TC.acct_nbr   = eqmt.acct_nbr)
                    WHERE 1 = 1
                    AND eqmt.p_yyyymmdd = '${hivevar:source_date}'
