...
#   Purpose:    : Functions to prepare gold_cpe_daily table in Gold layer
#                 1. Executes setup function to create gold_cpe_daily table in Gold Layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_CPE_DAILY_TBL}" \
     "${GOLD_CPE_DAILY_TBL}" \
...
#Transforming gold_cpe_daily table at gold layer
function fn_module_transform(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar gold_cpe_ivr_table="${GOLD_CPE_IVR_TBL}" \
   -hivevar gold_cpe_tc_table="${GOLD_CPE_TC_TBL}" \
