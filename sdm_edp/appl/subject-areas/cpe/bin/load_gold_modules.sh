...
if [[ $table_name == "cpe_daily" ]]
then
    job_name="EDP_CPE_${key_param_id}_DAILY_LD_GD"
elif [[ $table_name == "cpe_ivr" ]]
then
    job_name="EDP_CPE_${key_param_id}_IVR_LD_GD"
elif [[ $table_name == "cpe_monthly" ]]
then
    job_name="EDP_CPE_${key_param_id}_MONTHLY_LD_GD"
elif [[ $table_name == "cpe_tc" ]]
then
    job_name="EDP_CPE_${key_param_id}_TC_LD_GD"
elif [[ $table_name == "cpe_weekly" ]]
then
    job_name="EDP_CPE_${key_param_id}_WEEKLY_LD_GD"              
else
...
if [ $target_table == "gold_cpe_monthly" ]
then 
	  ...
	  source_month=`echo $begin_date | cut -c1-6`
  ...
  record_count=`beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} --showHeader=false --silent=true --outputformat=tsv2 -e \
             "SELECT COUNT(*) FROM $HIVE_DATABASE_NAME_GOLD.${HIVE_TABLE_PREFIX}${target_table} \
              WHERE  P_YYYYMM='$source_month';"`
              
elif [ $target_table == "gold_cpe_weekly" ]
then
