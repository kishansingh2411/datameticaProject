...
log_file_path=$LOG_DIR_SUBJECT_AREA/CPE_DEPLOYMENT_${current_timestamp}.log
...
do
   ...
   if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
    ...
    fn_log_info "Successfully create local directory ${local_dir} "  "${log_file_path}"
done
...
do
   ...
   if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
	...
	fn_log_info "Successfully create HDFS directory ${hdfs_dir} "  "${log_file_path}"
done
...
#######################################################################################
#                                                                                     #
#   Preparing Metadata tables			                                              #
#                                                                                     #
#######################################################################################   

tables=($INCOMING_CPE_MODEL_INFO_TBL)
for table in "${tables[@]}"; do  
    table_name=`basename ${table}`
