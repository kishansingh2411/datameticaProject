...
#   Purpose:    : Functions to prepare gold_cpe_monthly table in gold layer
#                 1. Executes setup function to create gold_cpe_monthly table in Gold Layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_CPE_MONTHLY_TBL}" \
     "${GOLD_CPE_MONTHLY_TBL}" \
...
#Transforming gold_cpe_monthly table at gold layer
function fn_module_transform(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   -hivevar begin_date=${begin_date} \
   -hivevar end_date=${source_date} \
   ...
   -hivevar gold_cpe_monthly_table="${GOLD_CPE_MONTHLY_TBL}" \
   -hivevar gold_cpe_daily_table="${GOLD_CPE_DAILY_TBL}" \
   -hivevar incoming_cpe_model_info_table="${INCOMING_CPE_MODEL_INFO_TBL}" \
