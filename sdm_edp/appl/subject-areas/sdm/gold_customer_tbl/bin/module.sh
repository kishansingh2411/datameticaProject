...
#   Purpose:    : Functions to prepare gold_customer table in Gold layer
#                 1. Executes setup function to create gold_customer table in Gold Layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_CUSTOMER_TBL}" \
     "${GOLD_CUSTOMER_TBL}" \
...
#Transforming gold_customer table at Gold layer
function fn_module_transform(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar gold_acct_hier_table="${GOLD_ACCT_HIER_FA_DIM_TBL}" \
