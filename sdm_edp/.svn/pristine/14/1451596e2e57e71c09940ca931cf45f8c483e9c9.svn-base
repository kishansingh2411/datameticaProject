...
#   Purpose:    : Functions to prepare gold_export_node_srvgrp table in Gold layer
#                 1. Executes setup function to create gold_export_node_srvgrp table in Gold Layer.
...
#   ------   --------------------     ----------       -------------------------------------
#    1.0     DataMetica Team          1/28/2017       Initial version
#
#
#####################################################################################################################
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_EXPORT_NODE_SRVGRP_TBL}" \
     "${GOLD_EXPORT_NODE_SRVGRP_TBL}" \
...
#Transforming gold_export_node_srvgrp table at gold layer
function fn_module_transform(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar gold_customer_service_groups_table="${GOLD_CUSTOMER_SERVICE_GROUPS_TBL}" \
