...
#   Purpose:    : Functions to prepare gold_export_cmts_node table in Gold layer
#                 1. Executes setup function to create gold_export_cmts_node table in Gold Layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_EXPORT_CMTS_NODE_TBL}" \
     "${GOLD_EXPORT_CMTS_NODE_TBL}" \
...
#Transforming gold_export_cmts_node_tbl table at Gold layer
function fn_module_transform(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar gold_export_cmts_node_table="${GOLD_EXPORT_CMTS_NODE_TBL}" \
   -hivevar gold_equipment_table="${GOLD_EQUIPMENT_TBL}" \
   -hivevar incoming_cmts_interface_mac_table="${INCOMING_CMTS_INTERFACE_MAC_TBL}" \
