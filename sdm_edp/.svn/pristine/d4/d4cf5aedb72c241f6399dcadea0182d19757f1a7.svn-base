...
INSERT OVERWRITE TABLE ${hivevar:gold_database}.${hivevar:table_prefix}${hivevar:gold_cpe_weekly_table}  PARTITION(P_YYYYMMDD)
SELECT 
	 CONCAT(substr('${hivevar:begin_date}',1,4), '-', substr('${hivevar:begin_date}',5,2), '-', substr('${hivevar:begin_date}',7,2)) BEGIN_date 
