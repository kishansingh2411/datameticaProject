...
#   Purpose:    : Functions to prepare incoming_item_dtl table in Incoming layer
#                 1. Executes setup function to create incoming_item_dtl table in Incoming Layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA}/${INCOMING_ITEM_DTL_TBL}" \
     "${INCOMING_ITEM_DTL_TBL}" \
...
#Transforming incoming_item_dtl table at incoming layer
function fn_module_import(){
  ...
  query=${QUERY_ITEM_DTL}
  query=${query//sysdate/$source_date}
  formatted_date=`echo $source_date | tr -d -`
  
  target_dir_path="${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA}/${INCOMING_ITEM_DTL_TBL}/p_yyyymmdd=$formatted_date" 
  ...
  if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
  then
      fn_log_error "Failed to drop to drop partition for table ${INCOMING_ITEM_DTL_TBL} " ${log_file_path}
      return $exit_code
  fi
  fn_log_info "Successfully drop partition if it exist for table ${INCOMING_ITEM_DTL_TBL}" "${log_file_path}" 
