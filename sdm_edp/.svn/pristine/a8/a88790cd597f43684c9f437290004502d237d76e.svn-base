...
success_message="Successfully generated batch id for IVR ingestion"  
failure_message="Failed to generate batch id for IVR ingestion, Quitting the process." 
fn_handle_exit_code "${exit_code}" "${success_message}" "${failure_message}" "${fail_on_error}" "${executor_log_file}"

##############################################################################
#                                                                            #
# Load incoming table incoming_cdr_resi                                      #
#                                                                            #
##############################################################################

sh $SUBJECT_AREA_HOME/bin/load_incoming_modules.sh $key_param_id cdr_resi
exit_code=$?

success_message="Successfully loaded table incoming_cdr_resi at Incoming layer"  
failure_message="Failed to load data in table incoming_cdr_resi at Incoming layer, Quitting the process." 
fn_handle_exit_code "${exit_code}" "${success_message}" "${failure_message}" "${BOOLEAN_TRUE}" "${executor_log_file}"

##############################################################################
#                                                                            #
# Load gold table gold_cdr_resi                                              #
#                                                                            #
##############################################################################

sh $SUBJECT_AREA_HOME/bin/load_gold_modules.sh $key_param_id cdr_resi
exit_code=$?

success_message="Successfully loaded table gold_cdr_resi at Gold layer"  
failure_message="Failed to load data in table gold_cdr_resi at Gold layer, Quitting the process." 
fn_handle_exit_code "${exit_code}" "${success_message}" "${failure_message}" "${BOOLEAN_TRUE}" "${executor_log_file}"
