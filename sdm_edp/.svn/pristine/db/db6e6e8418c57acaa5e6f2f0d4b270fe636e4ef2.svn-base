...
then
	  ...
	  filename=CpeMonthly$partition_value.txt
	...
	then
	  ...
	  filename=CpeWeekly$partition_value.txt
...
## Hadooop commmand to merge the multiple file to single file and place it at local

hadoop fs -getmerge "${source_dir}" "${target_dir}${filename}"
exit_code=$?   
	if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
	then
