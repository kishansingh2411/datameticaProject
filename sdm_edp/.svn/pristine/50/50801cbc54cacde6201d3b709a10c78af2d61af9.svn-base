...
#   Purpose:    : Functions to prepare outgoing_cpe_monthly_tbl table in Outgoing layer
#                 1. Executes setup function to create outgoing_cpe_monthly_tbl table in Outgoing Layer.
...
function fn_module_prepare(){
  ...
  dir_path="${DATA_LAYER_DIR_OUTGOING_SUBJECT_AREA}/${OUTGOING_CPE_MONTHLY_TBL}/p_yyyymm=$current_month/" 
   
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   -hivevar current_month=${current_month} \
