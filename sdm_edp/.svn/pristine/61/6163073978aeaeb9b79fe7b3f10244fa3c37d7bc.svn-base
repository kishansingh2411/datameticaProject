...
#   Purpose:    : Functions to prepare incoming_equipment_master table in Incoming layer
#                 1. Executes setup function to create incoming_equipment_master table in Incoming Layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA}/${INCOMING_EQUIPMENT_MASTER_TBL}" \
     "${INCOMING_EQUIPMENT_MASTER_TBL}" \
...
function fn_module_import(){
  ...
  query=${QUERY_EQUIPMENT_MASTER}
  query=${query//sysdate/$source_date}
  formatted_date=`echo $source_date | tr -d -`
  
  target_dir_path="${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA}/${INCOMING_EQUIPMENT_MASTER_TBL}/p_yyyymmdd=$formatted_date" 
  ...
  fn_alter_table_drop_one_partitions \
   ...
   if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
   then
      fn_log_error "Failed to drop to drop partition for table ${INCOMING_EQUIPMENT_MASTER_TBL} " ${log_file_path}
      return $exit_code
   fi
   fn_log_info "Successfully drop partition if it exist for table ${INCOMING_EQUIPMENT_MASTER_TBL}" "${log_file_path}" 
