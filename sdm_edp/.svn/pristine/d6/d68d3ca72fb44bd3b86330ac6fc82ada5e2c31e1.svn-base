...
job_name="EDP_FPT_${key_param_id}_CMTS_NOD_LD_OG"
...
sh $SUBJECT_AREA_HOME/${OUTGOING_EXPORT_CMTS_NODE_TBL}_tbl/bin/module.sh "prepare" "${source_date}" "${log_file_path}"
exit_code="$?"

if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
then
	fn_log_error "Failed to insert data at Outgoing Layer ${OUTGOING_EXPORT_CMTS_NODE_TBL}, Quitting the process" "${log_file_path}"	
	fn_update_job_statistics "${batch_id}" "${job_name}" "${start_time}" "NA" "${exit_code}" "${source_date}" "${log_file_path}" "" ""
fi
fn_log_info "Successfully inserted data at Outgoing Layer ${OUTGOING_EXPORT_CMTS_NODE_TBL}" "${log_file_path}"
...
record_count=`hadoop fs -cat ${DATA_LAYER_DIR_OUTGOING_SUBJECT_AREA}/${OUTGOING_EXPORT_CMTS_NODE_TBL}/p_yyyymmdd=${source_date}/* \
              | wc -l`
