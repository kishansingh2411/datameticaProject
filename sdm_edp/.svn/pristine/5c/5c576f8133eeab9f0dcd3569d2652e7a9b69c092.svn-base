...
#   Usage       : sh load_work_modules.sh key_Param_id table_name
#   Date        : 01/18/2017
...
if [[ $table_name == "acct_srv_occur_lob" ]]
then
    job_name="EDP_SDM_${key_param_id}_ACCT_SRV_LD_WK"
else
...
record_count=`beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} --showHeader=false --silent=true --outputformat=tsv2 -e \
             "SELECT COUNT(*) FROM $HIVE_DATABASE_NAME_WORK.${HIVE_TABLE_PREFIX}${target_table} \
