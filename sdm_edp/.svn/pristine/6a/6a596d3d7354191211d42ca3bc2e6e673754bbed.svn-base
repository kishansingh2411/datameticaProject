...
#   Purpose:    : Functions to prepare gold_cdr_resi_tbl table in Gold layer
#                 1. Executes setup function to create gold_cdr_resi_tbl table in Gold Layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_CDR_RESI_TBL}" \
     "${GOLD_CDR_RESI_TBL}" \
...
#Transforming gold_cdr_resi_tbl table at Gold layer
function fn_module_transform(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar gold_cdr_resi_table="${GOLD_CDR_RESI_TBL}" \
   -hivevar incoming_cdr_resi_table="${INCOMING_CDR_RESI_TBL}" \
