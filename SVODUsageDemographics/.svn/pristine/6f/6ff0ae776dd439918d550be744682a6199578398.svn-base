...
#   Purpose:    : Functions to prepare gold_d_vod_genre table in Gold layer
#                 1. Executes setup function to create gold_d_vod_genre table in Gold Layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_D_VOD_GENRE_TBL}" \
     "${GOLD_D_VOD_GENRE_TBL}" \
...
#Transforming gold_d_vod_genre table at Gold layer
function fn_module_transform(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar gold_table="${GOLD_D_VOD_GENRE_TBL}" \
   -hivevar incoming_table="${INCOMING_KOM_D_GENRE_TBL}" \
