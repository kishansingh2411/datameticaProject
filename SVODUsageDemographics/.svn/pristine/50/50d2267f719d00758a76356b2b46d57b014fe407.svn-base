...
firstDayStr=${year_month_id}${first}
...
fact_total=$(hive -e "SELECT count(*) FROM ${HIVE_DATABASE_NAME_GOLD}.${GOLD_F_VOD_ORDERS_MTH_CORP_TBL} WHERE month_id=${year_month_id};")
...
kom_f_vod_order=$(hive -e "SELECT count(*) FROM ${HIVE_DATABASE_NAME_INCOMING}.${INCOMING_KOM_F_VOD_ORDER_TBL} WHERE year_id=${year} and month_id=${month};")
kom_f_vod_order_formatted=$(printf "%'.f\n" $kom_f_vod_order)

fn_log_info "get the total count for the incoming table" "${log_file_path}"
#echo "get the total count for the incoming table"
kom_vod_order=$(hive -e "SELECT count(*) FROM ${HIVE_DATABASE_NAME_INCOMING}.${INCOMING_KOM_VOD_ORDER_TBL} WHERE month_id=${year_month_id};")
