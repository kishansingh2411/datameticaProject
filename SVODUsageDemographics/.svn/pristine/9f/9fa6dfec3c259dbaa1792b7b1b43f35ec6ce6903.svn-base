...
#   Purpose:    : Functions to prepare INCOMING_STG_D_GEOG_CORP_TBL table in incoming layer
#                 1. Executes setup function to create INCOMING_STG_D_GEOG_CORP_TBL table incoming Layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA}/${INCOMING_STG_D_GEOG_CORP_TBL}" \
     "${INCOMING_STG_D_GEOG_CORP_TBL}" \
...
function fn_module_transform(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar incoming_table="${INCOMING_STG_D_GEOG_CORP_TBL}" \
