...
job_name=EDP_SVOD_"${key_param_id}"_LD_CUST_ENCPT
...
##############################################################################
#																			 #
# Load kom customer account encrptor						 	 			 #
#																			 #
##############################################################################

jar_path=${SUBJECT_AREA_HOME}/lib/SVODUsageDemographics-1.0-SNAPSHOT.jar
output_KomCustomerAccountBulkEncryptor_dir="${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA}/${INCOMING_KOM_CUSTOMER_ACCOUNT_TBL}"

fn_hadoop_delete_directory_if_exists "${output_KomCustomerAccountBulkEncryptor_dir}" "${BOOLEAN_FALSE}" "${log_file_path}"
exit_code=$?
if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
then
   fn_log_error "Failed to delete directory ${output_KomCustomerAccountBulkEncryptor_dir}" ${log_file_path}
   return $exit_code
fi
fn_log_info "Successfully deleted directory ${output_KomCustomerAccountBulkEncryptor_dir}" ${log_file_path}
  
hadoop jar ${jar_path} com.alticeusa.ds.svodusagedemographics.encryptor.KomCustomerAccountBulkEncryptor "${output_KomCustomerAccountBulkEncryptor_dir}" 
exit_code=$?
if [ $exit_code == $EXIT_CODE_FAIL ]
then
   fn_log_error "Failed to get Customer Account encrypted data at directory ${output_KomCustomerAccountBulkEncryptor_dir}" "${log_file_path}"
   fn_update_job_statistics "${batch_id}" "${job_name}" "${start_time}" "NA" "${exit_code}" "${source_date}" "${log_file_path}" "" " "
fi
fn_log_info "Successfully got encrypted data at directory ${output_KomCustomerAccountBulkEncryptor_dir}" ${log_file_path}
