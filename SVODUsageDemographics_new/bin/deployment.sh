...
files=("dev_workflow.xml" \
		   "dev_coordinator.xml" \
		   "svodusagedemographics-schema.properties")
for file in "${files[@]}"
do
     if [[ ${file} == "dev_workflow.xml" ]] 
	  then  
		source_file_path="${SUBJECT_AREA_HOME}/oozie/dev_workflow.xml"
		target_directory="${HDFS_APP_PREFIX_SUBJECT_AREA_OOZIE}"
	  elif [[ ${file} == "dev_coordinator.xml" ]] 
	  then
	    source_file_path="${SUBJECT_AREA_HOME}/oozie/dev_coordinator.xml"
		target_directory="${HDFS_APP_PREFIX_SUBJECT_AREA_OOZIE}"
	 fi
