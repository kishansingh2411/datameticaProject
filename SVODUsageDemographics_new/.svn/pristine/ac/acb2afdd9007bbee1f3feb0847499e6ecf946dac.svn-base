...
#   Purpose:    : Functions to prepare gold_d_vod_title_tbl table in Gold layer
#                 1. Executes setup function to create gold_d_vod_title_tbl table in Gold Layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_D_VOD_TITLE_TBL}" \
     "${GOLD_D_VOD_TITLE_TBL}" \
...
#Transforming gold_d_vod_title table at Gold layer
function fn_module_transform(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar gold_table="${GOLD_D_VOD_TITLE_TBL}" \
