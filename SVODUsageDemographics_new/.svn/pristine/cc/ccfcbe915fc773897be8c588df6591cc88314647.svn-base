...
#   Purpose:    : Functions to prepare gold_d_vod_subscription_name table in Gold layer
#                 1. Executes setup function to create gold_d_vod_subscription_name table in Gold Layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${GOLD_D_VOD_SUBSCRIPTION_NAME_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_D_VOD_SUBSCRIPTION_NAME_TBL}" \
     "${GOLD_D_VOD_SUBSCRIPTION_NAME_TBL}" \
...
#Transforming gold_d_vod_subscription_name table at Gold layer
function fn_module_transform(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar gold_table="${GOLD_D_VOD_SUBSCRIPTION_NAME_TBL}" \
   -hivevar incoming_table="${INCOMING_KOM_D_SUBSCRIPTION_GROUP_TBL}" \
