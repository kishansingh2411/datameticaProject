...
#   Purpose:    : Functions to prepare incoming_stg3_vod_orders_mth_corp table in incoming layer
#                 1. Executes setup function to create incoming_stg3_vod_orders_mth_corp table incoming Layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${INCOMING_STG3_VOD_ORDERS_MTH_CORP_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA}/${INCOMING_STG3_VOD_ORDERS_MTH_CORP_TBL}" \
     "${INCOMING_STG3_VOD_ORDERS_MTH_CORP_TBL}" \
...
function fn_module_transform(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar incoming_stg2_vod_orders_mth_corp_table="${INCOMING_STG2_VOD_ORDERS_MTH_CORP_TBL}" \
   -hivevar incoming_stg3_vod_orders_mth_corp_table="${INCOMING_STG3_VOD_ORDERS_MTH_CORP_TBL}" \
   -hivevar incoming_vod_subscription_name_type_mapping_table="${INCOMING_VOD_SUBSCRIPTION_NAME_TYPE_MAPPING_TBL}" \
