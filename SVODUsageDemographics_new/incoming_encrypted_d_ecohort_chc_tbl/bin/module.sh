...
#   Purpose:    : Functions to prepare gold_encrypted_d_ecohort_chc table in Gold layer
#                 1. Executes setup function to create gold_encrypted_d_ecohort_chc table in Gold Layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${INCOMING_ENCRYPTED_D_ECOHORT_CHC_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA}/${INCOMING_ENCRYPTED_D_ECOHORT_CHC_TBL}" \
     "${INCOMING_ENCRYPTED_D_ECOHORT_CHC_TBL}" \
...
function fn_module_transform(){
  ...
  jar_path=${namenode_name}${HDFS_APP_PREFIX_SUBJECT_AREA_OOZIE_LIB}
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar incoming_table="${INCOMING_ENCRYPTED_D_ECOHORT_CHC_TBL}" \
   -hivevar gold_table="${GOLD_D_ECOHORT_CHC_TBL}" \
