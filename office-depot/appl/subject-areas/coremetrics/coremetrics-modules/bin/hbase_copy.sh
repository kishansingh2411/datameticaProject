...
hadoop fs -copyToLocal $OUTPUT_DIRECTORY/$1/$batchId/part-* $PROJECTS_HOME$OUTGOING/$1/$batchId/
...
#####Upserting data into Phoenix.   ##
	cd $PHOENIX_HOME
	./psql.py -t $hbase_table_name $zk_ip:$zk_port $arg > $PROJECTS_HOME$PROJ_TMP/$batchId/$logFileName 
...
######## Adding new cookie to the existing tables  #################
 file_list=(`hadoop fs -ls $COOKIE_HDFS/$batchId/part-* |awk '{split($0,a,"/"); print a[7]}'`)
  for i in "${file_list[@]}"
  do
   file=$i"_"$batchId"_cookies.csv"
   hadoop fs -mv $COOKIE_HDFS/$batchId/$i $COOKIE_HDFS/$batchId/$file
   hadoop fs -mv $COOKIE_HDFS/$batchId/$file $INCOMING_HDFS/$INCOMING_DIM_COOKIES/
  done
##############################################################################
#                                    End                                     #
##############################################################################
