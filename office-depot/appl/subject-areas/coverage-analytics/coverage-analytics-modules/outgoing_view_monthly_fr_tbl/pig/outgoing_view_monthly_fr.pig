...
--#                                                                            #
--# Name                                                                       #
--#     : outgoing_view_monthly_fr                                      	   #
--# File                                                                       #
--#     : outgoing_view_monthly_fr.pig                                         #
--# Description                                                                #
...
SET job.name '$OUTGOING_SCHEMA_FR_SSH_ACTION';
SET default_parallel 2;

-- Loading data from gold_coverage_analytics.gold_univ_tbl table.

gold_univ_tbl = LOAD '$GOLD_HDFS/$GOLD_UNIV/batch_id=$batch_id/country_cd=$country_code_fr' 
...
gold_cdmprddta_dm_tbl_filter =
   FILTER gold_cdmprddta_dm_tbl BY (batch_id == '$batch_id' AND country_cd == '$country_code_fr');
...
-- Storing records into /office_depot/coverage_analytics/outgoing/daily/batch_id/country_cd directory.
   ...
   INTO '$OUTPUT_DIRECTORY/$record_type/$batch_id/$country_code_fr'
   USING PigStorage(',');
