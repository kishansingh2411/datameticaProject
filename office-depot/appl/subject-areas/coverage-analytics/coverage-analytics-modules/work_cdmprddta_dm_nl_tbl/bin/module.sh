...
jobName=$WORK_CDMPRDDTA_DM_NL_SSH_ACTION
...
fn_insert_work_tbl "$debug_mode" "${logFilePath}" "$batchId" "$WORK_CDMPRDDTA_DM_NL"
...
fn_job_statistics "$startTime" "$endTime" "$exitCode" "$2" "$NOTAPPLICABLE" "$jobName" "$WORK_CDMPRDDTA_DM_NL" "$logFileName"
