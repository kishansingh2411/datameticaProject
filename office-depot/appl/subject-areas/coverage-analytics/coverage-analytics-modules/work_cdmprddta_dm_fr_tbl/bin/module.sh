...
jobName=$WORK_CDMPRDDTA_DM_FR_SSH_ACTION
...
fn_insert_work_tbl "$debug_mode" "${logFilePath}" "$batchId" "$WORK_CDMPRDDTA_DM_FR"
...
fn_job_statistics "$startTime" "$endTime" "$exitCode" "$1" "$NOTAPPLICABLE" "$jobName" "$WORK_CDMPRDDTA_DM_FR" "$logFileName"
