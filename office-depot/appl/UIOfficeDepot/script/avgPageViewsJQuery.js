(function($)
  {
       ...
       /*For Individual Country Selection This MEthod Is Called*/
     $.structCommonAvgPageViewPerVisit =function(data)
    {
    //  log(data);
      try{
       ...
       $(data.dataSet).each(function()
     {
       ...
       {"date": currentData.date,
        "totalPageViews": currentData.totalPageViews,
        "totalVisits": currentData.totalVisits,
...
//       log("Data By Country Formatted");
 ...
 $.getStructAvgPagePerVisitsForChart= function(dataCompute)
{
...
*/
        ...
        var totalVisits =parseFloat(dataCompute[loop].data[dataLoop].totalVisits);
        var totalPageViews = parseFloat(dataCompute[loop].data[dataLoop].totalPageViews);
/*        var visitorSegment =parseFloat(dataCompute[loop].data[dataLoop].visitorSegment);
*/        
        //OCR Compute totalOrders/totalVisits
        var avgPro =totalPageViews/totalVisits;
        log("Perform Value");
        log(dataCompute[loop].data[dataLoop].totalVisits);
        log(dataCompute[loop].data[dataLoop].totalPageViews);
        log(avgPro);
...
$.displayByCountryAvgPageViews =function(countryOrder,formattedDataGlobal)
    {
...
*/         tempObj.pointHighlightFill ='rgb(' + (Math.floor(Math.random() * 256)) + ',' + (Math.floor(Math.random() * 256)) + ',' + (Math.floor(Math.random() * 256)) +')';
        ...
        {  
          ...
          /*Ends*/
         var totalVisits =parseFloat(matchedCountry.data[loop].totalVisits);
         var totalPageViews = parseFloat(matchedCountry.data[loop].totalPageViews);
         var visitorSegment =parseFloat(matchedCountry.data[loop].visitorSegment);
         //OCR Compute totalOrders/totalVisits
          var ocr =totalPageViews/totalVisits;  
    ...
    $.formatToStackChartDataAvgPagePerVisit  = function(serialData)
    {
       ...
       while(loop!=eventLoop)
      {
       ...
       /* if(isNA)
        {  
          ocrNA = parseFloat(serialData.NA[loop].totalPageViews/serialData.NA[loop].totalVisits);
        }*/


        if(isDEFAULT)
        {  
          ocrDef = parseFloat(serialData.DEFAULT[loop].totalPageViews/serialData.DEFAULT[loop].totalVisits);
        }
        if(isEXISTINGCATALOGUECUSTOMER)
        {  
          ocrExCatCust = parseFloat(serialData.EXISTINGCATALOGUECUSTOMER[loop].totalPageViews/serialData.EXISTINGCATALOGUECUSTOMER[loop].totalVisits);
        }

        if(isNEWCUSTOMER)
        {  
         ocrNew  = parseFloat(serialData.NEWCUSTOMER[loop].totalPageViews/serialData.NEWCUSTOMER[loop].totalVisits);
        }
        if(isEXISTINGCUSTOMER)
        {  
         ocrExt  = parseFloat(serialData.EXISTINGCUSTOMER[loop].totalPageViews/serialData.EXISTINGCUSTOMER[loop].totalVisits);
        }
...
*/
      ...
      if(isDEFAULT)
      {
        ...
        while(loop!=eventLoop)
        {
          var totalPageViewsStack = parseInt(serialData.DEFAULT[loop].totalPageViews);
          var totalVisitsStack =  parseInt(serialData.DEFAULT[loop].totalVisits);
          var sum = totalPageViewsStack/totalVisitsStack;
          var finalAnn = ((sum/OCRSUMS[loop])*ocrGlobal[loop]);
          dataDefault.push(finalAnn);
          log("Avg page View Data Per Views DEFAULT");
          log("totalPageViews " +totalPageViewsStack);
          log("totalVisits "+totalVisitsStack);
          log("Sum : "+sum + "  Calculated : "+finalAnn);
          loop+=1;
        }
          var dataEnt ={};
          dataEnt.data =[];
       ...
       if(isEXISTINGCUSTOMER)
      {
        ...
        while(loop!=eventLoop)
        {
          var totalPageViewsStack = parseInt(serialData.EXISTINGCUSTOMER[loop].totalPageViews);
          var totalVisitsStack =  parseInt(serialData.EXISTINGCUSTOMER[loop].totalVisits);
          var sum = totalPageViewsStack/totalVisitsStack;
          var finalAnn = ((sum/OCRSUMS[loop])*ocrGlobal[loop]);
          dataExist.push(finalAnn);
          log("Avg page View Data Per Views EXISTINGCUSTOMER");
          log("totalPageViews " +totalPageViewsStack);
          log("totalVisits "+totalVisitsStack);
          log("Sum : "+sum + "  Calculated : "+finalAnn);
          loop+=1;
       ...
       if(isNEWCUSTOMER)
      {
        ...
        while(loop!=eventLoop)
        {
          var totalPageViewsStack = parseInt(serialData.NEWCUSTOMER[loop].totalPageViews);
          var totalVisitsStack =  parseInt(serialData.NEWCUSTOMER[loop].totalVisits);
          var sum = totalPageViewsStack/totalVisitsStack;
          var finalAnn = ((sum/OCRSUMS[loop])*ocrGlobal[loop]);
          dataNew.push(finalAnn);
          log("Avg page View Data Per Views NEWCUSTOMER");
          log("totalPageViews " +totalPageViewsStack);
          log("totalVisits "+totalVisitsStack);
          log("Sum : "+sum + "  Calculated : "+finalAnn);
          loop+=1;
       ...
       if(isEXISTINGCATALOGUECUSTOMER)
      {
        ...
        while(loop!=eventLoop)
        {
          var totalPageViewsStack = parseInt(serialData.EXISTINGCATALOGUECUSTOMER[loop].totalPageViews);
          var totalVisitsStack =  parseInt(serialData.EXISTINGCATALOGUECUSTOMER[loop].totalVisits);
          var sum = totalPageViewsStack/totalVisitsStack;
          var finalAnn = ((sum/OCRSUMS[loop])*ocrGlobal[loop]);
          dataExCatCust.push(finalAnn);
          log("Avg page View Data Per Views EXISTINGCATALOGUECUSTOMER");
          log("totalPageViews " +totalPageViewsStack);
          log("totalVisits "+totalVisitsStack);
          log("Sum : "+sum + "  Calculated : "+finalAnn);
          loop+=1;
      ...
      chart.datasets = datasets;
          log("Chart Stacked Data For AvgPage Views");
          log(chart);
          return chart;
