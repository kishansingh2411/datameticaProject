(function($)
	{
     ...
     $.structCommonPercentRevnExistVisitors =function(data)
    {
    //  log(data);
      try{
       ...
       $(data.dataSet).each(function()
     {
       ...
       {"date": currentData.date,
        "totalRevenueFromExistingCustomers": currentData.totalRevenueFromExistingCustomers,
...
//       log("Data By Country Formatted");
 ...
 $.getStructPercentRevExistVisitors= function(dataCompute)
{
...
*/
        ...
        var totalRevenueFromExistingCustomers =parseFloat(dataCompute[loop].data[dataLoop].totalRevenueFromExistingCustomers);
...
*/        
        ...
        var avgPro =totalRevenueFromExistingCustomers/totalRevenue;
        log("Perform Value");
        log(avgPro);
...
catch(e)
{
   ...
   /* get Country Based on DropDown Selection*/
    $.displayByCountryChartPercentRevExistVisitors=function(countryOrder,formattedDataGlobal)
    {
...
*/         tempObj.pointHighlightFill ='rgb(' + (Math.floor(Math.random() * 256)) + ',' + (Math.floor(Math.random() * 256)) + ',' + (Math.floor(Math.random() * 256)) +')';
        ...
        {  
          ...
          /*Ends*/
         var totalRevenueFromExistingCustomers =parseFloat(matchedCountry.data[loop].totalRevenueFromExistingCustomers);
         var totalRevenue = parseFloat(matchedCountry.data[loop].totalRevenue);
         //OCR Compute totalOrders/totalVisits
          var ocr =totalRevenue/totalRevenueFromExistingCustomers;  
