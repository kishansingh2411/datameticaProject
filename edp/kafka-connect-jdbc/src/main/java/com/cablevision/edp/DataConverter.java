...
public class DataConverter {
  ...
  public static Schema convertSchema(String tableName, ResultSetMetaData metadata)
    ...
    log.debug("In method convertSchema....calling method addFieldSchema.....");
  ...
  private static void addFieldSchema(ResultSetMetaData metadata, int col,
    ...
    log.debug("In method addFieldSchema....got label ["+label+"] name ["+name+"] fieldName ["+fieldName+"] sqlType ["+sqlType+"]");
    ...
    switch (sqlType) {
      ...
      case Types.NUMERIC:
      case Types.DECIMAL: {
    	  log.debug("In method addFieldSchema....got type as NUMERIC or DECIMAL...");
        //SchemaBuilder fieldBuilder = Decimal.builder(metadata.getScale(col));
        //if (optional) {
        //  fieldBuilder.optional();
    	  final int scale = metadata.getScale(col);
    	  log.debug("In method addFieldSchema....got scale ["+scale+"] for column "+metadata.getColumnName(col));
    	  if (scale == 0 || scale == -127) {
    	  if (optional) {
    		  log.debug("In method addFieldSchema....In option 1........");
    	   builder.field(fieldName, Schema.OPTIONAL_INT64_SCHEMA);
    	  } else {
    		  log.debug("In method addFieldSchema....In option2.......");
    	  builder.field(fieldName, Schema.INT64_SCHEMA);
    	  }
    	  } else {
    		  log.debug("In method addFieldSchema....In option3........");
    ...
    switch (colType) {
      ...
      case Types.DECIMAL: {
        //colValue = resultSet.getBigDecimal(col);
    	  ...
    	  log.debug("In method convertFieldValue...got scale..."+scale+" for column "+metaData.getColumnName(col));
    	  if (scale == 0 || scale == -127) {
//    		  log.info("In method convertFieldValue...option1....");
    	      colValue = resultSet.getLong(col);
    	  }else {
//    		  log.info("In method convertFieldValue...option2....");
    		  if (log.isDebugEnabled())
    			  log.debug("getting value from big decimal column "+metaData.getColumnName(col));
    		 
    	     colValue = resultSet.getBigDecimal(col);
    	     if (log.isDebugEnabled())
    	    	 log.debug("decimal column value for column "+metaData.getColumnName(col)+" is "+colValue);
    	     
    	     if (colValue != null)
    	     {
    	    	 log.debug("setting scale as "+scale +" for column "+metaData.getColumnName(col));
    	    	 colValue = ((BigDecimal)colValue).setScale(scale);
    	    	 log.debug("scale set as "+((BigDecimal)colValue).scale() +" for column "+metaData.getColumnName(col));
    	     }
    	  }
        break;
      }
      ...
      // Timestamp is a date + time
      case Types.TIMESTAMP: {
    	  log.debug("result set obj having columns "+resultSet.getMetaData().getColumnCount()+ "for "+col+" is "+resultSet);
        colValue = resultSet.getTimestamp(col);
        log.debug("column value for columns "+resultSet.getMetaData().getColumnCount()+" is "+colValue);
        break;
      }
    ...
    log.debug("In method convertFieldValue.....for fieldName ["+fieldName+"] colValue ["+(resultSet.wasNull() ? null : colValue)+"]");
