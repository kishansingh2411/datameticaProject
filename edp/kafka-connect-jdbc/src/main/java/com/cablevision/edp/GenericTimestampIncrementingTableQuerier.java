...
public class GenericTimestampIncrementingTableQuerier extends CustomTableQuerier {
  ...
  protected void createPreparedStatement(Connection db) throws SQLException {
    ...
    log.debug("Prepared SQL query: "+queryString);
    log.debug("{} prepared SQL query: {}", this, queryString);
    log.debug("connection used "+db);
...
@Override
  protected ResultSet executeQuery() throws SQLException {
	log.debug("In executeQuery method");
	...
	{
		i = 0;
		log.debug("querying on load date "+loadDate+" for table "+name);
    ...
    log.debug("Executing query for table "+name);
