...
//          validateNonNullable(mode, tableOrQuery, incrementingColumnList, timestampColumnList, whiteListTables);
  ...
  {
	  log.debug("Committing record offset for source partition "+record.sourcePartition() +", "+record.sourceOffset());
	  offsetMap.put(record.sourcePartition(), record.sourceOffset());
	  log.debug("offset map "+offsetMap);
  }
  ...
  public void stop() throws ConnectException {
    ...
    if (db != null) {
      log.info("Closing the source database connection");
      try {
        db.close();
      } catch (SQLException e) {
    ...
    if (offsetDb != null) {
        log.info("Closing the audit database connection");
        try {
        	offsetDb.close();
        } catch (SQLException e) {
