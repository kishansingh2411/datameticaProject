...
import com.cablevision.edp.JdbcSourceConnectorConstants;
...
public class JdbcSourceTaskUpdateTest extends JdbcSourceTaskTestBase {
  ...
  public void testManualIncrementingRestoreOffset() throws Exception {
    ...
    offset.put(CustomJdbcSourceTask.INCREMENTING_FIELD, 1L);
  ...
  public void testTimestampRestoreOffset() throws Exception {
    Map<String, Object> offset = new HashMap<>();
    offset.put(CustomJdbcSourceTask.TIMESTAMP_FIELD, 10L);
    expectInitialize(Arrays.asList(SINGLE_TABLE_PARTITION),
                     Collections.singletonMap(SINGLE_TABLE_PARTITION, offset));
  ...
  public void testTimestampAndIncrementingRestoreOffset() throws Exception {
    Map<String, Object> offset = new HashMap<>();
    offset.put(CustomJdbcSourceTask.TIMESTAMP_FIELD, 10L);
    offset.put(CustomJdbcSourceTask.INCREMENTING_FIELD, 3L);
    expectInitialize(Arrays.asList(SINGLE_TABLE_PARTITION),
                     Collections.singletonMap(SINGLE_TABLE_PARTITION, offset));
    ...
    for (SourceRecord record : records) {
      ...
      switch (field) {
        ...
        case INCREMENTING_OFFSET:
          extracted = (T)(record.sourceOffset()).get(CustomJdbcSourceTask.INCREMENTING_FIELD);
          break;
        case TIMESTAMP_OFFSET: {
          java.util.Date rawTimestamp
              = (java.util.Date) record.sourceOffset().get(CustomJdbcSourceTask.TIMESTAMP_FIELD);
          extracted = (T) (Long) rawTimestamp.getTime();
          break;
        }
        default:
  ...
  private void assertIncrementingOffsets(List<SourceRecord> records) {
    ...
    for(SourceRecord record : records) {
      ...
      long offsetValue = (Long)(record.sourceOffset())
          .get(CustomJdbcSourceTask.INCREMENTING_FIELD);
  ...
  private void assertTimestampOffsets(List<SourceRecord> records) {
    ...
    for(SourceRecord record : records) {
      ...
      long offsetValue = (Long)(record.sourceOffset()
          .get(CustomJdbcSourceTask.TIMESTAMP_FIELD));
