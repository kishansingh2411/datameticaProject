...
#   Usage       : load_incoming_modules.sh "key_param_id" "table_name" 
#   Date        : 04/12/2016
...
then
    job_name="EDP_MRDVR_600006_INC_PRF"
elif [[ $table_name == "rsdvr_recordings_archive" ]]
then
    job_name="EDP_MRDVR_600006_INC_RCA"
elif [[ $table_name == "rsdvr_recordings" ]]
then
    job_name="EDP_MRDVR_600006_INC_REC"
elif [[ $table_name == "rsdvr_requests_archive" ]]
then
    job_name="EDP_MRDVR_600006_INC_RQA"
elif [[ $table_name == "rsdvr_requests" ]]
then
    job_name="EDP_MRDVR_600006_INC_REQ"
elif [[ $table_name == "rsdvr_schedule" ]]
then
    job_name="EDP_MRDVR_600006_INC_SCH"
else
...
target_table=${HIVE_TABLE_PREFIX}"incoming_${table_name}"

sh ${SUBJECT_AREA_HOME}/${target_table}_tbl/bin/module.sh "prepare" "${log_file_path}" "${dataset}" "${source_date}"
exit_code=$?   

if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
then
	fn_log_error "For date ${source_date}, failed to load table "${target_table}" in Incoming layer" "${log_file_path}"
	fn_update_job_statistics "${BATCH_ID}" "${job_name}" "${start_time}" "0" "${exit_code}" "${source_date}" "${log_file_path}" "" " "
fi 

fn_log_info "For date ${source_date}, successfully loaded table ${target_table} in Incoming layer" "${log_file_path}"
...
hive_counts="$(fn_get_hive_count "${HIVESERVER2_URL}" "${HIVE_USENAME}" "${HIVE_PASSWORD}" "${HIVE_DATABASE_NAME_INCOMING}" ${target_table} "1" "${source_date}")"
exit_code=$?
