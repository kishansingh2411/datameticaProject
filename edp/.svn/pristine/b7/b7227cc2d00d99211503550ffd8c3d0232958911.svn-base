...
#                 1. Executes setup function to prepare incoming_t_f_household_device_tbl table in incoming layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${INCOMING_T_F_HOUSEHOLD_DEVICE_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_T_F_HOUSEHOLD_DEVICE_TBL}" \
     "${INCOMING_T_F_HOUSEHOLD_DEVICE_TBL}" \
...
#Preparing incoming_t_f_household_device_tbl table.
...
function fn_module_prepare(){   
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar source_table="${WORK_T_F_HOUSEHOLD_DEVICE_TBL}" \
   -hivevar target_table="${INCOMING_T_F_HOUSEHOLD_DEVICE_TBL}" \
