...
#                    and create gold_metadata_int_nsn_call_usage_tbl in gold layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${GOLD_METADATA_INT_NSN_CALL_USAGE_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_METADATA_INT_NSN_CALL_USAGE_TBL}" \
     "${GOLD_METADATA_INT_NSN_CALL_USAGE_TBL}" \
...
#Preparing a_dom_nsn_in_usage  table.
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar gold_metadata_table="${GOLD_METADATA_INT_NSN_CALL_USAGE_TBL}" \
...
#Transforming gold_metadata_int_nsn_call_usage  table.
function fn_module_transform(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar incoming_table="${INCOMING_INT_NSN_CALL_USAGE_TBL}" \
