...
#                 1. Executes setup function to prepare incoming_account_tns table in incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_CUSTOMER}/${INCOMING_ACCOUNT_TNS_TBL}" \
     "${INCOMING_ACCOUNT_TNS_TBL}" \
...
function fn_module_import(){
   ...
   src_table_name=`echo ${INCOMING_ACCOUNT_TNS_TBL} | cut -d'_' -f2- | tr '[:lower:]' '[:upper:]'`
   ...
   target_dir_path="${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_CUSTOMER}/${INCOMING_ACCOUNT_TNS_TBL}/source_date="${source_date}"/*"
