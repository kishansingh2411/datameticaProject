...
#                 1. Executes setup function to prepare work_rsdvr_recordings_archive_tmp table in work layer.
#               		Input Arguments for this script are: Phase name (setup), Log file path(/log/file/path)                 										  
#                 2. Executes transform function to prepare work_rsdvr_recordings_tmp table in work layer.
...
#   ------   --------------------     ----------       -------------------------------------
#    1.0     DataMetica Team          31/08/2016       Initial version
#
#
#####################################################################################################################
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${WORK_RSDVR_RECORDINGS_ARCHIVE_TMP_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_RECORDINGS_ARCHIVE_TMP_TBL}" \
     "${WORK_RSDVR_RECORDINGS_ARCHIVE_TMP_TBL}" \
...
#Preparing work_rsdvr_recordings_tmp table.
function fn_module_transform(){
   ...
   beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      ...
      -hivevar work_rsdvr_recordings_archive_tmp_tbl=${WORK_RSDVR_RECORDINGS_ARCHIVE_TMP_TBL} \
      -hivevar hive_database_name_incoming=${HIVE_DATABASE_NAME_INCOMING} \
      -hivevar incoming_rsdvr_recordings_archive_tbl=${INCOMING_RSDVR_RECORDINGS_ARCHIVE_TBL} \
      ...
      -hivevar dtm_start=${dtm_start} \
      -hivevar dtm_end=${dtm_end} \
      -hivevar default_date="${DEFAULT_DATE}" \
