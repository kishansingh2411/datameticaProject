...
sh $SUBJECT_AREA_HOME/bin/load_incoming_unified_modules.sh "${key_param_id}"
exit_code=$?

success_message="Successfully loaded incoming Customer Service Last Rec table."  
failure_message="Failed to load data for incoming ECR9 Shopping cart table, Quitting the process." 
fn_handle_exit_code "${exit_code}" "${success_message}" "${failure_message}" "${fail_on_error}" "${executor_log_file}"
