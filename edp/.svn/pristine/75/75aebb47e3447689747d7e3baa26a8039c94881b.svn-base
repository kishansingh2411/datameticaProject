...
#   Purpose:    : Module script to load gold vow esp hit data table
#                 1. Executes setup function to prepare directory structure for the table											                                						  
#                    and create gold_vow_hit_data_esp_tbl in gold layer.
...
. ${HOME}/subject-areas/ods/etc/ods.properties
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_VOW_HIT_DATA_ESP_TBL}" \
     "${GOLD_VOW_HIT_DATA_ESP_TBL}" \
...
# Deriving vow esp hit data table at Gold layer
function fn_module_transform(){
    ...
    pig -useHCatalog -p suite_name=$suite_name \
		...
		-p gold_h_optimum_user_tbl="${HIVE_TABLE_PREFIX}${GOLD_H_OPTIMUM_USER_TBL}" \
		...
		-p incoming_referrer_type_tbl="${HIVE_TABLE_PREFIX}${INCOMING_REFERRER_TYPE_TBL}" \
		-p incoming_search_engines_tbl="${HIVE_TABLE_PREFIX}${INCOMING_SEARCH_ENGINES_TBL}" \
		-p incoming_optimum_pagenames_tbl="${HIVE_TABLE_PREFIX}${INCOMING_OPTIMUM_PAGENAMES_TBL}" \
		-p incoming_optimum_site_sections_tbl="${HIVE_TABLE_PREFIX}${INCOMING_OPTIMUM_SITE_SECTIONS_TBL}" \
        -p incoming_optimum_isp_tbl="${HIVE_TABLE_PREFIX}${INCOMING_OPTIMUM_ISP_TBL}" \
		-p gold_vow_hit_data_tbl="${HIVE_TABLE_PREFIX}${GOLD_VOW_HIT_DATA_TBL}" \
 		-p gold_vow_hit_data_esp_tbl="${HIVE_TABLE_PREFIX}${GOLD_VOW_HIT_DATA_ESP_TBL}" \
        -p incoming_error_hit_data_tbl="${HIVE_TABLE_PREFIX}${INCOMING_ERROR_HIT_DATA_TBL}" \
