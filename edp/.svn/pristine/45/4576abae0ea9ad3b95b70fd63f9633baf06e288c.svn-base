...
#                 1. Executes setup function to prepare gold_bhv_acct_phone_nbr_opt table in gold layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${GOLD_BHV_ACCT_PHONE_NBR_OPT_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA_CUSTOMER}/${GOLD_BHV_ACCT_PHONE_NBR_OPT_TBL}" \
     "${GOLD_BHV_ACCT_PHONE_NBR_OPT_TBL}" \
...
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar work_bhv_acct_phone_nbr_opt_tmp=${HIVE_TABLE_PREFIX}${WORK_BHV_ACCT_PHONE_NBR_OPT_TMP_TBL} \
   -hivevar gold_database=${HIVE_DATABASE_NAME_GOLD_ODS} \
   -hivevar gold_bhv_acct_phone_nbr_opt="${HIVE_TABLE_PREFIX}${GOLD_BHV_ACCT_PHONE_NBR_OPT_TBL}" \
