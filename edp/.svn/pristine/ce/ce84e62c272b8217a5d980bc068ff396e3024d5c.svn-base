...
if [ ! "$#" -gt 0 ] && [ ! "$#" -le 2 ]
then
  echo "Illegal number of parameters.  Parameters expected [key_param_id is_historic_load(Optional)]"
  exit 
fi
...
job_name="OVCDR_${key_param_id}_BATCH_ID_GEN"
...
if [ -z "$is_historic_load" ]
then
    source_file_name=${dataset_name}_${source_date}.tar.gz
else
	source_file_name=""
fi
...
#      1. log file									                         #	
#	   2. logging batch Id of provided dataset name							 #
#																			 #
##############################################################################
