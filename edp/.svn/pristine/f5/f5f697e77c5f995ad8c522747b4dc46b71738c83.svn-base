...
#                    and create work_code999_tbl in work layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_CODE999_TBL}" \
     "${WORK_CODE999_TBL}" \
...
function fn_module_prepare(){   
 ...
 #Sqoop EDP_CODE999_TBL table data
   ...
   then
      fn_log_error "Failed while recreating the table ${WORK_CODE999_TBL}" "${log_file_path}"
      return $exit_code
   fi
