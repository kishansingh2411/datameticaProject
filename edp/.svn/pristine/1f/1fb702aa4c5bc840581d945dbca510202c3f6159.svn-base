...
#                    and create incoming_wipoutlet_tbl in incoming layer.
...
#				: Transform - 
#						sh module.sh "prepare" "${max_seq_id}" "${source_table}" "${log_file_path}" ${load_start_date} ${load_end_date}
#   Date        : 05/02/2016
#   Log File    : 
#   SQL File    : 
#   Error File  : 
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_WIPOUTLET_TBL}" \
     "${INCOMING_WIPOUTLET_TBL}" \
...
#Preparing incoming EDP_WIPOUTLET_TBL table.
function fn_module_prepare(){
  ...
  #Sqoop EDP_WIPOUTLET_TBL table data
