...
#                 1. Executes setup function to prepare incoming_zipmaster_tbl table in incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_ZIPMASTER_TBL}" \
     "${INCOMING_ZIPMASTER_TBL}" \
...
#Preparing incoming zipmaster  table.
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar source_table="${WORK_ZIPMASTER_TBL}" \
   -hivevar target_table="${INCOMING_ZIPMASTER_TBL}" \
