...
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar work_table="${WORK_A_INT_RAD_CALL_USAGE_TBL}" \
   -hivevar incoming_table="${INCOMING_STG_INT_RAD_CALL_USAGE_TBL}" \
   -hivevar gold_table="${GOLD_A_INT_RAD_CALL_USAGE_TBL}" \
   -hivevar work_chc_table="${WORK_CHC_INT_RAD_TBL}" \
