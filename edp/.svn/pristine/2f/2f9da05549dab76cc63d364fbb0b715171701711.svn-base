...
#                 1. Executes setup function to prepare incoming_rsdvr_requests_tbl table in incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_RSDVR_REQUESTS_TBL}" \
     "${INCOMING_RSDVR_REQUESTS_TBL}" \
...
function fn_module_prepare(){
  ...
  fn_mrdvr_execute_pig \
    ...
    "${HIVE_TABLE_PREFIX}${WORK_RSDVR_REQUESTS_TBL}" \
    "${HIVE_DATABASE_NAME_INCOMING}" \
    "${HIVE_TABLE_PREFIX}${INCOMING_RSDVR_REQUESTS_TBL}" \
    "${PIG_UDF_JAR_PATH}" \
    "${CVS_JAR_PATH}" \
    "${source_date}" \
    "${version}" \
