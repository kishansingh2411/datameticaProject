...
#                 1. Executes setup function to create work_bhv_acct_phone_nbr_opt_tmp table in work layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${WORK_BHV_ACCT_PHONE_NBR_OPT_TMP_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_BHV_ACCT_PHONE_NBR_OPT_TMP_TBL}" \
     "${WORK_BHV_ACCT_PHONE_NBR_OPT_TMP_TBL}" \
...
#Preparing work_bhv_acct_phone_nbr_opt_tmp optimum table.
function fn_module_prepare(){

   source_date=$1
   log_file_path=$2

   target_dir_path="${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_BHV_ACCT_PHONE_NBR_OPT_TMP_TBL}/*"
   ...
   -hivevar work_bhv_acct_phone_nbr_opt_tbl="${WORK_BHV_ACCT_PHONE_NBR_OPT_TBL}" \
   -hivevar work_bhv_acct_phone_nbr_opt_tmp_tbl="${WORK_BHV_ACCT_PHONE_NBR_OPT_TMP_TBL}" \
   -hivevar gold_bhv_acct_phone_nbr_opt_tbl="${GOLD_BHV_ACCT_PHONE_NBR_OPT_TBL}" \
   -hivevar dtm_expired_default="${dtm_expired_default}" \
   -f "${hql_file}" 1>> "${log_file_path}" 2>> "${log_file_path}" 
   exit_code=$?
   if [ ${exit_code} -ne ${EXIT_CODE_SUCCESS} ]
   then
      fn_log_error "Failed to load data in ${WORK_BHV_ACCT_PHONE_NBR_OPT_TMP_TBL} table." "${log_file_path}"
      return $exit_code
   else
      fn_log_info "Successfully loaded data in ${WORK_BHV_ACCT_PHONE_NBR_OPT_TMP_TBL} table." "${log_file_path}"        
   fi
