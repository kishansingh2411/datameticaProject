...
#                 1. Executes setup function to prepare work_cust_services_last_rec_merge_tbl table in work layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${WORK_CUST_SERVICES_LAST_REC_MERGE_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_CUST_SERVICES_LAST_REC_MERGE_TBL}" \
     "${WORK_CUST_SERVICES_LAST_REC_MERGE_TBL}" \
...
#Preparing work_cust_service_last_rec_merge table.
function fn_module_prepare(){
  log_file_path="$1"
  current_date=`date +"%Y-%m-%d"`
  target_dir_path="${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_CUST_SERVICES_LAST_REC_MERGE_TBL}"  
  ...
  fn_hadoop_create_directory_if_not_exists "${target_dir_path}" "${BOOLEAN_FLASE}" "${log_file_path}"
  exit_code=$?
  
  if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
  then
      fn_log_error "Failed to create directory  ${target_dir_path}" ${log_file_path}
      return $exit_code
  fi  
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   -hivevar source_database1="${HIVE_DATABASE_NAME_INCOMING}" \
   -hivevar source_database2="${HIVE_DATABASE_NAME_GOLD}" \
   -hivevar target_database="${HIVE_DATABASE_NAME_WORK}" \
   -hivevar table_prefix="${HIVE_TABLE_PREFIX}" \
   -hivevar source_table1="${INCOMING_CUST_SERVICES_LAST_REC_TBL}" \
   -hivevar source_table2="${GOLD_CUST_SERVICES_LAST_REC_TBL}" \
   -hivevar source_date="${current_date}" \
   -hivevar target_table="${WORK_CUST_SERVICES_LAST_REC_MERGE_TBL}" \
  ...
  if [ ${exit_code} -ne ${EXIT_CODE_SUCCESS} ]
  then
     fn_log_error "Error occurred while performing merge between table ${HIVE_DATABASE_NAME_INCOMING}.${INCOMING_CUST_SERVICES_LAST_REC_TBL} and ${HIVE_DATABASE_NAME_GOLD}.${GOLD_CUST_SERVICES_LAST_REC_TBL} " ${log_file_path}
     return $exit_code
  fi	
