...
#   Department: AM
#
#   Program name: calculate_aggregate_pageview_counts.sh
#   Program type: Unix Bash Shell script
#   Purpose:    : This will calculate the PageView aggregates and store them in gold layer.
#                 Input Arguments for this script are: key_Param_id, frequency(daily).               
#   Author(s)   : DataMetica Team
#   Usage       : sh calculate_aggregate_pageview_counts.sh key_Param_id "daily"
#   Date        : 12/28/2015
...
if [ "$#" -ne 2 ]; then
  ...
  echo "Parameters expected [ key_param_id frequency(daily) ]"
  exit 
fi
...
then
	job_name="UOW_600002_DLY_AGGR_PGVW_CNT"
elif [ $suite_name == "$ONET_PROD" ]
then
  	job_name="ONET_600009_DLY_AGGR_PGVW_CNT"
else
...
if [ $frequency == "daily" ]
then
   date_range=`beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} --showHeader=false --silent=true \
               --outputformat=tsv2 -e" SELECT DAY_TAG FROM $HIVE_DATABASE_NAME_INCOMING.${HIVE_TABLE_PREFIX}${INCOMING_PERIOD_TBL} \
...
##############################################################################
#                                                                            #
# Calculating PageView Counts aggregate for provided frequency               #
#                                                                            #
##############################################################################

args="START_DATE=$period_start_date | END_DATE=$period_end_date | MEASURE=${MEASURE_PAGE_VIEW} | LEVEL = $(echo "$frequency" | tr '[:lower:]' '[:upper:]') "

fn_log_info "Calculating PageView Counts aggregates for suite $suite_name and for date $source_date" ${log_file_path}

sh $SUBJECT_AREA_HOME/gold_aggregate_pageview_counts_tbl/bin/module.sh "transform" "${suite_name}" \
   "${period_start_date}" "${period_end_date}" "${frequency}" "${duration}" "${log_file_path}"
exit_code="$?"

if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
then
	fn_log_error "Failed to calculate $frequency PageView Counts aggregates for suite $suite_name and date $source_date, Quitting the process" "${log_file_path}"
	fn_update_job_statistics "${batch_id}" "${job_name}" "${start_time}" "NA" "${exit_code}" "${source_date}" \
	  "${log_file_path}" "${source_file_name}" "$args"
fi

fn_log_info "Successfully calculated $frequency PageView Counts aggregates for gold layer" "${log_file_path}"
