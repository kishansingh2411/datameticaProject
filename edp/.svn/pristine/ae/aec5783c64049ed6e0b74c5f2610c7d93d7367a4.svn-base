...
#                 1. Executes setup function to prepare incoming_auxcust_tbl table in incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_AUXCUST_TBL}" \
     "${INCOMING_AUXCUST_TBL}" \
...
#Preparing incoming auxcust table.
...
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar source_table="${WORK_AUXCUST_TBL}" \
   -hivevar target_table="${INCOMING_AUXCUST_TBL}" \
