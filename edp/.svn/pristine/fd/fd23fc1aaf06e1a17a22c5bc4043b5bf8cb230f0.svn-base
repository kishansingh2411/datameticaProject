...
#                 1. Executes setup function to prepare gold_rsdvr_recordings_tbl table in gold layer.
#               		Input Arguments for this script are: Phase name (setup), Log file path(/log/file/path)                 										  
#                 2. Executes prepare function.
#               		Input Arguments for this script are: "prepare" "${source_table}" "${log_file_path}"
#				  3. Executes transform function to prepare gold_rsdvr_recordings_delta_tbl table in gold layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${GOLD_RSDVR_RECORDINGS_DELTA_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_RSDVR_RECORDINGS_DELTA_TBL}" \
     "${GOLD_RSDVR_RECORDINGS_DELTA_TBL}" \
...
#importing incoming_rsdvr_recordings table.
...
function fn_module_import(){
  ...
  query="$(fn_build_query_for_import_from_netezza t_${source_table} "${COL_RSDVR_RECORDINGS_DELTA}")"
...
#Preparing gold_rsdvr_recordings_delta table.
...
beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      ...
      -hivevar gold_rsdvr_recordings_delta=${GOLD_RSDVR_RECORDINGS_DELTA_TBL} \
...
#Deleting gold_rsdvr_recordings_delta table.
