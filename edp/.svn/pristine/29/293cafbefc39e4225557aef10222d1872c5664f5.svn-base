...
#                 1. Executes setup function to prepare incoming_techs_tbl table in incoming layer.
#               		Input Arguments for this script are: Phase name (setup), Log file path(/log/file/path)                 										  
#                 2. Executes prepare function.
#               		Input Arguments for this script are: "prepare" "${max_seq_id}" "${query}" "${source_table}" "${log_file_path}")
#   Author(s)   : DataMetica Team
#   Usage       : setup - 
#						sh module.sh "setup" "/local/file/path" 
#				: Prepare - 
#						sh module.sh "prepare" ""${max_seq_id}" "${source_table}" "/local/file/path"
#   Date        : 05/02/2016
#   Log File    : 
#   SQL File    : 
#   Error File  : 
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_TECHS_TBL}" \
     "${INCOMING_TECHS_TBL}" \
...
#Preparing incoming EDP_TECHS_TBL table.
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar source_table="${WORK_TECHS_TBL}" \
   -hivevar target_table="${INCOMING_TECHS_TBL}" \
