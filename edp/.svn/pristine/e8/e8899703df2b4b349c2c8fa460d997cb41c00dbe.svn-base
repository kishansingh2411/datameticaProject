...
--#   Purpose:    : Create Work table(work_rsdvr_recordings_tmp_tbl)
...
(  		
   ...
   STARTDATE_STARTTIME_EST_GOLD         TIMESTAMP COMMENT'Instance derived from gold table',
   STARTDATE_STARTTIME_EST_INCOMING     TIMESTAMP COMMENT'rectime in Eastern Time',
   STARTDATE_STARTTIME_EST_IND	        VARCHAR(1) COMMENT'Column value modify indicator',
   ARCHIVE_TIME_GOLD        	        TIMESTAMP COMMENT'Instance derived from gold table',
   ARCHIVE_TIME_INCOMING	            TIMESTAMP COMMENT'start_time in Eastern Time',
   ARCHIVE_TIME_IND	                    VARCHAR(1) COMMENT'Column value modify indicator',
   ...
   ARCHIVE_DATE_EST_GOLD	            DATE COMMENT'Instance derived from gold table',
   ARCHIVE_DATE_EST_INCOMING	        DATE COMMENT'',
   ARCHIVE_DATE_EST_IND	                VARCHAR(1) COMMENT'Column value modify indicator',
   LOAD_DATE                            TIMESTAMP COMMENT '',
   SOURCE_LOAD_DATE_GOLD                DATE COMMENT '',
   SOURCE_LOAD_DATE_INCOMING            DATE COMMENT '',
   SOURCE_LOAD_DATE_IND                 VARCHAR(1) COMMENT '',
   STARTDATE_EST_GOLD	                DATE COMMENT'Instance derived from gold table',
   STARTDATE_EST_INCOMING	            DATE COMMENT'date and time data loaded into the DB',
   STARTDATE_EST_IND	                VARCHAR(1) COMMENT'Column value modify indicator',
   ...
   PAUSE_TIME_GOLD	                    INT COMMENT'Instance derived from gold table',
   PAUSE_TIME_INCOMING	                INT COMMENT'the time of the pause that initiated this temporary recording. this will only be populated for recordings that were saved after a pause action',
   ...
   RECORDING_LOCKED_GOLD	            INT COMMENT'Instance derived from gold table',
   RECORDING_LOCKED_INCOMING	        INT COMMENT'specified whether the recording is locked (parental controls)',
   ...
   RECORDING_TIME_GOLD	                INT  COMMENT'Instance derived from gold table',
   RECORDING_TIME_INCOMING	            INT  COMMENT'Start time of the recording',
   ...
   START_TIME_GOLD	                    INT COMMENT'Instance derived from gold table',
   START_TIME_INCOMING	                INT COMMENT'time recording is to start',
