...
#                 1. Executes setup function to prepare work_customer_account_tbl table in work layer.
...
function fn_module_prepare(){
  ...
  target_dir_path="${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_CUSTOMER_ACCOUNT_TBL}" 
  ...
  query="select /*+ NO_PARALLEL(a) */ a.*  FROM ${ORACLE_DATABASE_OVCDR}.${source_table} a \
     WHERE a.DTM_CREATED  >= TO_DATE('${dtm_last_updated}','yyyy-mm-dd HH24:MI:SS') \
     AND a.DTM_CREATED  < TO_DATE('${current_date}','yyyy-mm-dd HH24:MI:SS') \
     OR  a.DTM_LAST_UPDATED >= TO_DATE('${dtm_last_updated}','yyyy-mm-dd HH24:MI:SS') \
     AND a.DTM_LAST_UPDATED < TO_DATE('${current_date}','yyyy-mm-dd HH24:MI:SS') \
     AND \$CONDITIONS"
