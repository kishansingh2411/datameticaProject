...
#                 Input Arguments for this script is: key_Param_id and operation.               
#   Author(s)   : DataMetica Team
#   Usage       : sh load_gold_modules.sh  key_Param_id operation
#   Date        : 12/19/2016
...
then 
    ...
    job_name="EDP_${key_param_id}_INSRT_GOLD_MDULS"
elif [[ $operation == update ]]
then
    operation_name="prepare"
    job_name="EDP_${key_param_id}_UPD_GOLD_MDULS"
elif [[ $operation == delete ]] 
then
   operation_name="delete"  
   job_name="EDP_${key_param_id}_DEL_GOLD_MDULS"
fi
...
sh $SUBJECT_AREA_HOME/${table_name}_tbl/bin/module.sh "$operation_name" "${source_date}" "${log_file_path}"
exit_code=$?

if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
then
   fn_log_error "For dataset $dataset_name, failed to load gold table ${table_name}, Quitting the process" "${log_file_path}"
   fn_update_job_statistics "${batch_id}" "${job_name}" "${start_time}" "NA" "${exit_code}" "${source_date}" "${log_file_path}" "${source_file_name}" " "
fi   

fn_log_info "For dataset $dataset_name, successfully loaded table ${table_name}" "${log_file_path}"
...
if [[ $operation == insert ]]
then 
    record_count=$(fn_get_count "${HIVESERVER2_URL}" "${HIVE_USENAME}" "${HIVE_PASSWORD}" "${HIVE_DATABASE_NAME_GOLD}" "${table_name}" "I" "${source_date}")
    exit_code=$?
elif [[ $operation == update ]]
then
    record_count=$(fn_get_count "${HIVESERVER2_URL}" "${HIVE_USENAME}" "${HIVE_PASSWORD}" "${HIVE_DATABASE_NAME_GOLD}" "${table_name}" "U" "${source_date}")
    exit_code=$?
elif [[ $operation == delete ]] 
then  
   record_count="NA"
