...
then
    job_name="EDP_MRDVR_600006_WRK_PRF_DEL_D"
elif [[ $table_name == "rsdvr_recordings" ]]
then
    job_name="EDP_MRDVR_600006_WRK_REC_DEL_D"
elif [[ $table_name == "rsdvr_requests" ]]
then
    job_name="EDP_MRDVR_600006_WRK_REQ_DEL_D"
elif [[ $table_name == "rsdvr_schedule" ]]
then
    job_name="EDP_MRDVR_600006_WRK_SCH_DEL_D"
elif [[ $table_name == "rsdvr_requests_archive" ]]
then
    job_name="EDP_MRDVR_600006_WRK_RQA_DEL_D"
elif [[ $table_name == "rsdvr_recordings_archive" ]]
then
    job_name="EDP_MRDVR_600006_WRK_RCA_DEL_D"
else
...
##############################################################################
#																			 #
# Loading work seq table                                                     #
#																			 #
##############################################################################

target_table=${HIVE_TABLE_PREFIX}"work_${table_name}_delete_dedup"
...
then
	fn_log_error "For date ${source_date}, failed while executing dedup for delete functionality for "${target_table}" in work layer" "${log_file_path}"
	fn_update_job_statistics "${BATCH_ID}" "${job_name}" "${start_time}" "0" "${exit_code}" "${source_date}" "${log_file_path}" "" " "
fi 

fn_log_info "For date ${source_date}, successfully executed dedup for delete functionality for ${target_table} in Work layer" "${log_file_path}"
