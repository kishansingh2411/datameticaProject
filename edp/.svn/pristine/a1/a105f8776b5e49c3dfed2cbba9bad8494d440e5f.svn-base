...
#                 1. Executes setup function to prepare incoming_ip_sip_did_phone_nbr_tbl table in incoming layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${INCOMING_IP_SIP_DID_PHONE_NBR_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_CUSTOMER}/${INCOMING_IP_SIP_DID_PHONE_NBR_TBL}" \
     "${INCOMING_IP_SIP_DID_PHONE_NBR_TBL}" \
...
function fn_module_prepare(){
  ...
  source_table=${ORACLE_DATABASE_OVCDR}.${IP_SIP_DID_PHONE_NBR_SOURCE_TBL}
  log_file_path=$1
  connection_url="${DRIVER}${HOST_OVCDR}:${PORT_OVCDR}:${SCHEMA_OVCDR}"
  target_dir_path="${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_CUSTOMER}/${INCOMING_IP_SIP_DID_PHONE_NBR_TBL}" 
