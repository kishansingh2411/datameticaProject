...
#                 1. Executes setup function to prepare gold_rsdvr_requests_archive_last_rec_tbl table in gold layer.
#               		Input Arguments for this script are: Phase name (setup), Log file path(/log/file/path)                 										  
#                 2. Executes prepare function.
#               		Input Arguments for this script are: "prepare" "${source_table}" "${log_file_path}"
#				  3. Executes transform function to prepare gold_rsdvr_requests_archive_last_rec_tbl table in gold layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${GOLD_RSDVR_REQUESTS_ARCHIVE_LAST_REC_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_RSDVR_REQUESTS_ARCHIVE_LAST_REC_TBL}" \
     "${GOLD_RSDVR_REQUESTS_ARCHIVE_LAST_REC_TBL}" \
...
#Preparing gold_rsdvr_requests_last_rec table.
...
#Preparing gold_rsdvr_requests_archive_last_rec_tbl table.
...
beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      ...
      -hivevar gold_rsdvr_requests_archive_last_rec_tbl=${GOLD_RSDVR_REQUESTS_ARCHIVE_LAST_REC_TBL} \
      -hivevar hive_database_name_work=${HIVE_DATABASE_NAME_WORK} \
      -hivevar work_rsdvr_requests_archive_dedup_tbl=${WORK_RSDVR_REQUESTS_ARCHIVE_DEDUP_TBL} \
...
beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      -hivevar hive_database_name_gold=${HIVE_DATABASE_NAME_GOLD} \
      -hivevar gold_rsdvr_requests_archive_last_rec=${GOLD_RSDVR_REQUESTS_ARCHIVE_LAST_REC_TBL} \
      -hivevar hive_database_name_work=${HIVE_DATABASE_NAME_WORK} \
      -hivevar work_rsdvr_requests_archive_delete_dedup=${WORK_RSDVR_REQUESTS_ARCHIVE_DELETE_DEDUP_TBL} \
      -f ${hive_script_path} >> ${log_file_path} 2>&1;
      exit_code=$?
      if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
	  then
