...
#                 1. Executes setup function to prepare gold_rsdvr_schedule_delta_tbl table in incoming layer.
#               		Input Arguments for this script are: Phase name (setup), Log file path(/log/file/path)                 										  
#                 2. Executes prepare function.
#               		Input Arguments for this script are: "prepare" "${source_table}" "${log_file_path}"
# 				  3. Executes transform function to prepare gold_rsdvr_schedule_delta_tbl table in gold layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_RSDVR_SCHEDULE_DELTA_TBL}" \
     "${GOLD_RSDVR_SCHEDULE_DELTA_TBL}" \
...
#Preparing incoming_rsdvr_schedule table.
...
function fn_module_import(){
  ...
  query="$(fn_build_query_for_import_from_netezza t_${source_table} "${COL_RSDVR_SCHEDULE_DELTA}")"
 ...
 #Sqoop gold_rsdvr_schedule_delta table data
...
#Preparing gold_rsdvr_schedule_delta table.
...
beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      ...
      -hivevar gold_rsdvr_schedule_delta=${GOLD_RSDVR_SCHEDULE_DELTA_TBL} \
      -hivevar hive_database_name_work=${HIVE_DATABASE_NAME_WORK} \
      -hivevar work_rsdvr_schedule_dedup=${WORK_RSDVR_SCHEDULE_DEDUP_TBL} \
...
#Preparing gold_rsdvr_schedule_delta_tbl table.
...
beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      -hivevar hive_database_name_gold=${HIVE_DATABASE_NAME_GOLD} \
      -hivevar gold_rsdvr_schedule_delta=${GOLD_RSDVR_SCHEDULE_DELTA_TBL} \
      -hivevar gold_rsdvr_schedule_last_rec=${GOLD_RSDVR_SCHEDULE_LAST_REC_TBL} \
      -hivevar hive_database_name_work=${HIVE_DATABASE_NAME_WORK} \
      -hivevar work_rsdvr_schedule_delete_dedup=${WORK_RSDVR_SCHEDULE_DELETE_DEDUP_TBL} \
