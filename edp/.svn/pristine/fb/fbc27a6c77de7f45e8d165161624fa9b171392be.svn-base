...
#                 1. Executes setup function to prepare incoming_slsmn_tbl table in incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_SLSMN_TBL}" \
     "${INCOMING_SLSMN_TBL}" \
...
#Preparing incoming EDP_SLSMN_TBL table.
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar source_table="${WORK_SLSMN_TBL}" \
   -hivevar target_table="${INCOMING_SLSMN_TBL}" \
