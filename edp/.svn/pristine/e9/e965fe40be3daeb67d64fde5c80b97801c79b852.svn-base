...
then
    job_name="EDP_${key_param_id}_GD_VMA_NSN_CALL"
elif [[ $table_name == "a_dom_nsn_out_usage" ]]
then
    job_name="EDP_${key_param_id}_GD_DOM_NSN_OUT"	
elif [[ $table_name == "a_int_rad_call_usage" ]]
  then
    job_name="EDP_${key_param_id}_GD_INT_RAD_CALL"
elif [[ $table_name == "a_int_nsn_call_usage" ]]
then
    job_name="EDP_${key_param_id}_GD_INT_NSN_CALL"
elif [[ $table_name == "a_dom_nsn_in_usage" ]]
then
    job_name="EDP_${key_param_id}_GD_DOM_NSN_IN"
else
...
sh ${SUBJECT_AREA_HOME}/gold_${table_name}_tbl/bin/module.sh "prepare" ${source_date} "${log_file_path}" 
exit_code=$?   
...
record_count=`beeline -u ${HIVESERVER2_URL} \
              ...
              "SELECT COUNT(*) FROM ${HIVE_DATABASE_NAME_GOLD}.${HIVE_TABLE_PREFIX}GOLD_${table_name};"`
