...
success_message="Successfully generated batch id for key_param_id $key_param_id"  
failure_message="Failed to generate batch id for key_param_id $key_param_id, Quitting the process." 
...
sh $SUBJECT_AREA_HOME/bin/load_work_modules.sh $key_param_id delete
exit_code=$?
...
sh $SUBJECT_AREA_HOME/bin/modify_gold_modules.sh $key_param_id
exit_code=$?

success_message="Successfully modified data in Gold layer"
failure_message="Failed while modified data in Gold, Quitting the process."
fn_handle_exit_code "${exit_code}" "${success_message}" "${failure_message}" "${fail_on_error}" "${executor_log_file}"
...
sh $SUBJECT_AREA_HOME/bin/load_gold_modules.sh $key_param_id delete
exit_code=$?
...
sh $SUBJECT_AREA_HOME/bin/load_gold_modules.sh $key_param_id insert
exit_code=$?
...
sh $SUBJECT_AREA_HOME/bin/load_gold_modules.sh $key_param_id update
exit_code=$?
...
sh $SUBJECT_AREA_HOME/bin/load_gold_metadata.sh $key_param_id
exit_code=$?

success_message="Successfully loaded gold metadata table"
failure_message="Failed while loading gold metadata, quitting the process."
fn_handle_exit_code "${exit_code}" "${success_message}" "${failure_message}" "${fail_on_error}" "${executor_log_file}"
...
sh $SUBJECT_AREA_HOME/bin/compute_statistics.sh $key_param_id  1>> ${executor_log_file} 2>> ${executor_log_file}
exit_code=$?
 
success_message="Successfully loaded data after computing statistics for key_param_id $key_param_id"  
failure_message="Failed while loading data for computed statistics for key_param_id $key_param_id, Quitting the process." 
