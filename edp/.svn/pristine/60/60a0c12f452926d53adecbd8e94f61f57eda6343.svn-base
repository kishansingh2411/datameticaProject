...
#                    and create gold_metadata_dom_nsn_in_usage_tbl in gold layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_METADATA_DOM_NSN_IN_USAGE_TBL}" \
     "${GOLD_METADATA_DOM_NSN_IN_USAGE_TBL}" \
...
#Preparing gold_metadata_nsn_in_usage  table.
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar gold_metadata_table="${GOLD_METADATA_DOM_NSN_IN_USAGE_TBL}" \
...
#Transforming gold_metadata_nsn_in_usage  table.
function fn_module_transform(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar incoming_table="${INCOMING_DOM_NSN_IN_USAGE_TBL}" \
