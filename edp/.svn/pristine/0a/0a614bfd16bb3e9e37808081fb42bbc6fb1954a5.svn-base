...
--#   Purpose:    : Loading data from incoming_hit_data table to gold_vow_hit_data table
...
incoming_hit_data_filter_with_condtion = 
    ...
    BY (post_page_event == 0 AND hit_source != 5 AND hit_source != 8 AND hit_source != 9 AND exclude_hit <= 0)
;
...
SPLIT hit_data_decrypted_records 
    ...
    IF (
    	TRIM(evar11_optimum_id) is null OR TRIM(evar11_optimum_id) == ''
    	),
...
hit_data_records_join_plugins = 
   FOREACH hit_data_records_join_isp 
   GENERATE ..plugins, 
   ResolvePluginId(plugins, incoming_plugins_generate.plugin_rec_bag) AS plugins_name, 
   post_browser_height..
;
...
STORE generated_records
  INTO '${hive_database_name_gold}.${gold_vow_hit_data_tbl}'
