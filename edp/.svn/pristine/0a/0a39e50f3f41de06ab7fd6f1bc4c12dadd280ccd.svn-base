...
#   Usage       : load_incoming_bhv_modules.sh  
#   Date        : 06/02/2017
...
#   ------   --------------------     ----------       -------------------------------------
#    1.0     DataMetica Team          06/02/2017       Initial version
#
#
#####################################################################################################################
...
if [ "${table_name}" == "account_info" ]
then
 job_name="EDP_ODS_${key_param_id}_LD_IN_ACC_INFO"
elif [ "${table_name}" == "account_tns" ]
then
  job_name="EDP_ODS_${key_param_id}_LD_IN_ACC_TNS"
elif [ "${table_name}" == "cust_telephone_nbr" ]
then
  job_name="EDP_ODS_${key_param_id}_LD_IN_CUST_NBR"
elif [ "${table_name}" == "customer_telephone" ]
then
  job_name="EDP_ODS_${key_param_id}_LD_IN_CUST_TN"
fi
...
sh ${SUBJECT_AREA_HOME}/${target_table}_tbl/bin/module.sh "import" "${source_date}" "${log_file_path}"
exit_code=$?   
...
hive_counts=`beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   --showHeader=false --silent=true \
   --outputformat=tsv2 -e "SELECT COUNT(*) \
                           FROM ${HIVE_DATABASE_NAME_INCOMING_ODS}.${HIVE_TABLE_PREFIX}${target_table};"`                                             
exit_code=$?
