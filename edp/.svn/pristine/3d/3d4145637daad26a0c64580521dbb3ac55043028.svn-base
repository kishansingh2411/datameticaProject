...
#                 1. Executes setup function to prepare incoming_t_f_split_ipchannel_tuning_tbl table in incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_T_F_SPLIT_IPCHANNEL_TUNING_TBL}" \
     "${INCOMING_T_F_SPLIT_IPCHANNEL_TUNING_TBL}" \
...
#Preparing incoming_t_f_split_ipchannel_tuning_tbl table.
function fn_module_prepare(){   
  ...
  fn_distcp_from_s3 \
  	...
  	"${NUM_MAPPERS}" \
  	"${source_dir}" \
  	"${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_T_F_SPLIT_IPCHANNEL_TUNING_TBL}/broadcast_month=${load_month}" \
  	"${log_file_path}"
  
  fn_alter_table_with_one_partitions "${HIVE_DATABASE_NAME_INCOMING}" \
	"${HIVE_TABLE_PREFIX}${INCOMING_T_F_SPLIT_IPCHANNEL_TUNING_TBL}" \
