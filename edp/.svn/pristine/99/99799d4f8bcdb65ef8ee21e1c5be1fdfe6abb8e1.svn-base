...
#                    and create incoming_raterptctrs_tbl in incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_RATERPTCTRS_TBL}" \
     "${INCOMING_RATERPTCTRS_TBL}" \
...
#Preparing INCOMING_RATERPTCTRS user table.
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar source_table="${WORK_RATERPTCTRS_TBL}" \
   -hivevar target_table="${INCOMING_RATERPTCTRS_TBL}" \
