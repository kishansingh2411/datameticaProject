...
#   Department: AM
#
#   Program name: load_work_history_data.sh
...
if [[ $table_name == "a_vma_nsn_call_usage" ]]
then
    job_name="OVCDR_${key_param_id}_WK_VMA_NSN_CALL"
elif [[ $table_name == "a_dom_nsn_out_usage" ]]
then
    job_name="OVCDR_${key_param_id}_WK_DOM_NSN_OUT"	
elif [[ $table_name == "a_int_rad_call_usage" ]]
  then
    job_name="OVCDR_${key_param_id}_WK_INT_RAD_CALL"
elif [[ $table_name == "a_int_nsn_call_usage" ]]
then
    job_name="OVCDR_${key_param_id}_WK_INT_NSN_CALL"
elif [[ $table_name == "a_dom_nsn_in_usage" ]]
then
    job_name="OVCDR_${key_param_id}_WK_DOM_NSN_IN"
else
...
##############################################################################
#																			 #
# Loading work historic data                                                 #
#																			 #
##############################################################################
 
sh ${SUBJECT_AREA_HOME}/work_${table_name}_tbl/bin/module.sh "import" "${table_name}" "${log_file_path}" 
exit_code=$?   

if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
then
	fn_log_error "Failed to load "${table_name}" table historic data in work layer" "${log_file_path}"
	fn_update_job_statistics "${batch_id}" "${job_name}" "${start_time}" "0" "${exit_code}" "${source_date}" "${log_file_path}" "" " "
fi 

fn_log_info "Successfully loaded ${table_name} table historic data in work layer" "${log_file_path}"

##############################################################################
#																			 #
# Capture record counts for this load                                        #
#																			 #
##############################################################################
 
record_count=`beeline -u ${HIVESERVER2_URL} \
              -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
              --showHeader=false --silent=true --outputformat=tsv2 -e \
              "SELECT COUNT(*) FROM ${HIVE_DATABASE_NAME_WORK}.${HIVE_TABLE_PREFIX}WORK_${table_name};"`
