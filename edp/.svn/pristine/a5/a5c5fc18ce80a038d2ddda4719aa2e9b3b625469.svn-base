...
#                 1. Executes setup function to prepare work_nz_t_f_split_channel_tuning_rst_tbl table in work layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${WORK_NZ_T_F_SPLIT_CHANNEL_TUNING_RST_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_NZ_T_F_SPLIT_CHANNEL_TUNING_RST_TBL}" \
     "${WORK_NZ_T_F_SPLIT_CHANNEL_TUNING_RST_TBL}" \
...
#Preparing work_nz_t_f_split_channel_tuning_rst_tbl table.
...
function fn_module_prepare(){   
  ...
  target_dir="${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_NZ_T_F_SPLIT_CHANNEL_TUNING_RST_TBL}"
  start_date="$3"
  end_date="$4"
 ...
 #Sqoop WORK_NZ_T_F_SPLIT_CHANNEL_TUNING_RST_TBL table data
   ...
   if [ ${exit_code} -ne ${EXIT_CODE_SUCCESS} ]
   then
      fn_log_error "Failed while recreating the table ${WORK_NZ_T_F_SPLIT_CHANNEL_TUNING_RST_TBL}" "${log_file_path}"
      return -1
   fi
