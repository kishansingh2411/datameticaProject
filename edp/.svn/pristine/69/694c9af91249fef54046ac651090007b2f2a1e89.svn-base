...
#                    and create stg_dom_nsn_out_call_tbl at Incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_DOM_NSN_OUT_USAGE_TBL}" \
     "${INCOMING_DOM_NSN_OUT_USAGE_TBL}" \
...
function fn_module_prepare(){
  ...
  to_directory="${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_DOM_NSN_OUT_USAGE_TBL}/source_date=${source_date}"
  ...
  fn_alter_table_with_one_partitions \
   ...
   if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
   then
      fn_log_error "Failed to alter table $INCOMING_DOM_NSN_OUT_USAGE_TBL for the source_date=${source_date}  , Quitting the prossess..!!!" "${log_file_path}"
       return $exit_code
   fi
