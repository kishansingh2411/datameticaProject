...
#   Usage       : load_gold_metdata.sh "key_param_id" "table_name"
#   Date        : 01/20/2017
...
then
   table_name="vma_nsn_call_usage"
   job_name="EDP_${key_param_id}_GD_MD_VMA_NSN_CALL"
    
elif [[ $dataset_name == "dom_nsn_out" ]]
then
   table_name="dom_nsn_out_usage"
   job_name="EDP_${key_param_id}_GD_MD_DOM_NSN_OUT"	
  
elif [[ $dataset_name == "int_rad" ]]
  then
   table_name="int_rad_call_usage"
   job_name="EDP_${key_param_id}_GD_MD_INT_RAD_CALL"
   
elif [[ $dataset_name == "int_nsn" ]]
then
   table_name="int_nsn_call_usage"
   job_name="EDP_${key_param_id}_GD_MD_INT_NSN_CALL"
   
elif [[ $dataset_name == "dom_nsn_in" ]]
then
   table_name="dom_nsn_in_usage"
   job_name="EDP_${key_param_id}_GD_MD_DOM_NSN_IN"
...
sh ${SUBJECT_AREA_HOME}/gold_metadata_${table_name}_tbl/bin/module.sh "transform" "${log_file_path}" "${source_date}" 
exit_code=$?   

if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
then
	fn_log_error "Failed to load gold_metadata_${table_name} table for ${source_date} source_date data in gold layer" "${log_file_path}"
	fn_update_job_statistics "${batch_id}" "${job_name}" "${start_time}" "0" "${exit_code}" "${source_date}" "${log_file_path}" "" " "
fi 

fn_log_info "Successfully loaded gold_metadata_${table_name} table for ${source_date} source_date data data in gold layer" "${log_file_path}"
