...
#                 1. Executes setup function to prepare incoming_customer_account table in incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_CUSTOMER}/${INCOMING_CUSTOMER_ACCOUNT_TBL}" \
     "${INCOMING_CUSTOMER_ACCOUNT_TBL}" \
...
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar source_table="${WORK_CUSTOMER_ACCOUNT_TBL}" \
   -hivevar target_table="${INCOMING_CUSTOMER_ACCOUNT_TBL}" \
