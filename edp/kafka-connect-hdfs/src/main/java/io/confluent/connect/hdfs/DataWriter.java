...
public class DataWriter {
  ...
  public void syncWithHive() throws ConnectException {
    ...
    try {
      for (String topic: topics) {
        String topicDir = FileUtils.topicDirectory(url, topicsDir, connectorConfig.getList(HdfsSinkConnectorConfig.DESTINATION_SUBDIRS_CONFIG).
    			get(connectorConfig.getList(HdfsSinkConnectorConfig.TOPICS_CONFIG).indexOf(topic)));
        ...
        if (fileStatusWithMaxOffset != null) {
          ...
          hive.createTable(hiveDatabase, connectorConfig.getList(HdfsSinkConnectorConfig.HIVE_TABLES_CONFIG).
        			get(connectorConfig.getList(HdfsSinkConnectorConfig.TOPICS_CONFIG).indexOf(topic)), latestSchema, partitioner);
          List<String> partitions = hiveMetaStore.listPartitions(hiveDatabase, connectorConfig.getList(HdfsSinkConnectorConfig.HIVE_TABLES_CONFIG).
        			get(connectorConfig.getList(HdfsSinkConnectorConfig.TOPICS_CONFIG).indexOf(topic)), (short) -1);
          ...
          for (FileStatus status : statuses) {
            ...
            if (!partitions.contains(location)) {
              ...
              hiveMetaStore.addPartition(hiveDatabase, connectorConfig.getList(HdfsSinkConnectorConfig.HIVE_TABLES_CONFIG).
          			get(connectorConfig.getList(HdfsSinkConnectorConfig.TOPICS_CONFIG).indexOf(topic)), partitionValue);
            }
          }
        }
      }
  ...
  private Map<String, Object> copyConfig(HdfsSinkConnectorConfig config) {
    ...
    map.put(HdfsSinkConnectorConfig.TOPICS_DIR_CONFIG, config.getString(HdfsSinkConnectorConfig.TOPICS_DIR_CONFIG));
    map.put(HdfsSinkConnectorConfig.DESTINATION_SUBDIRS_CONFIG, config.getList(HdfsSinkConnectorConfig.DESTINATION_SUBDIRS_CONFIG));
    map.put(HdfsSinkConnectorConfig.TOPICS_CONFIG, config.getList(HdfsSinkConnectorConfig.TOPICS_CONFIG));
    map.put(HdfsSinkConnectorConfig.HIVE_TABLES_CONFIG, config.getList(HdfsSinkConnectorConfig.HIVE_TABLES_CONFIG));
    map.put(HdfsSinkConnectorConfig.TABLE_PARTITION_LEVEL, config.getString(HdfsSinkConnectorConfig.TABLE_PARTITION_LEVEL));
    return map;
  }
}
