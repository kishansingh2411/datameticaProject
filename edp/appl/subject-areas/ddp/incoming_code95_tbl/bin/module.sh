...
#                    and create incoming_code95_tbl in incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_CODE95_TBL}" \
     "${INCOMING_CODE95_TBL}" \
...
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar source_table="${WORK_CODE95_TBL}" \
   -hivevar target_table="${INCOMING_CODE95_TBL}" \
