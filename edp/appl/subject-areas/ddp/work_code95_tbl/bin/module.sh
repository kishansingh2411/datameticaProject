...
#                    and create work_code95_tbl in work layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_CODE95_TBL}" \
     "${WORK_CODE95_TBL}" \
...
#Preparing incoming EDP_CODE95_TBL table.
function fn_module_prepare(){   
 ...
 #Sqoop EDP_CODE95_TBL table data
   ...
   then
      fn_log_error "Failed while recreating the table ${WORK_CODE95_TBL}" "${log_file_path}"
      return $exit_code
   fi
