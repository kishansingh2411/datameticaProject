...
#                 1. Executes setup function to prepare incoming_wipcustrate_tbl table in incoming layer.
...
#				: Prepare - 
#						sh module.sh "prepare" "${max_seq_id}" "${source_table}" "/local/file/path" ${load_start_date} ${load_end_date}
#   Date        : 05/02/2016
#   Log File    : 
#   SQL File    : 
#   Error File  : 
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_WIPCUSTRATE_TBL}" \
     "${INCOMING_WIPCUSTRATE_TBL}" \
...
#Preparing incoming EDP_WIPCUSTRATE_TBL table.
function fn_module_prepare(){
  ...
  query="$(fn_build_query_for_import ${source_table} ${ORACLE_DATABASE} ${max_seq_id} ${LARGE_TBL} ${load_start_date} ${load_end_date})"
 ...
 #Sqoop EDP_WIPCUSTRATE_TBL table data
