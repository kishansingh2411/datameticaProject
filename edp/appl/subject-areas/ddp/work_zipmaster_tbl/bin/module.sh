...
#                    and create work_zipmaster_tbl in work layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_ZIPMASTER_TBL}" \
     "${WORK_ZIPMASTER_TBL}" \
...
#Preparing incoming EDP_ZIPMASTER_TBL table.
function fn_module_prepare(){   
 ...
 #Sqoop EDP_ZIPMASTER_TBL table data
   ...
   then
      fn_log_error "Failed while recreating the table ${WORK_ZIPMASTER_TBL}" "${log_file_path}"
      return $exit_code
   fi
