...
#                    and create incoming_ratecodes_tbl in incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_RATECODES_TBL}" \
     "${INCOMING_RATECODES_TBL}" \
...
#Preparing incoming EDP_RATECODES_TBL table.
...
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar source_table="${WORK_RATECODES_TBL}" \
   -hivevar target_table="${INCOMING_RATECODES_TBL}" \
