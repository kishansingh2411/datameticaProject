...
#   Department: AM
#
#   Program name: load_gold_bhv_modules.sh
#   Program type: Unix Bash Shell script
#   Purpose:    : This will load data in tables of Gold layer.
#                 Input Arguments for this script are: "key_param_id"              
#   Author(s)   : DataMetica Team
#   Usage       : load_gold_bhv_modules.sh  
#   Date        : 02/16/2017
...
then
 job_name="EDP_ODS_${key_param_id}_LD_GD_BHV_OPT"
elif [ "${table_name}" == "bhv_acct_phone_nbr_sdl" ]
then
  job_name="EDP_ODS_${key_param_id}_LD_GD_BHV_SDL"
elif [ "${table_name}" == "bhv_acct_phone_nbr_opt_sdl" ]
then
  job_name="EDP_ODS_${key_param_id}_LD_GD_BHV_OPT_SDL"
fi
...
target_table_gold="gold_${table_name}"
...
sh ${SUBJECT_AREA_HOME}/${target_table_gold}_tbl/bin/module.sh "prepare" "${log_file_path}" "${source_date}"
exit_code=$?   
...
fn_log_info "Successfully loaded ${target_table_gold} in Gold layer" "${log_file_path}"
...
hive_counts=`beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} --showHeader=false --silent=true \
			--outputformat=tsv2 -e "SELECT COUNT(*) FROM ${HIVE_DATABASE_NAME_GOLD_ODS}.${HIVE_TABLE_PREFIX}${target_table_gold} \
             WHERE TO_DATE(DTM_CREATED) = CURRENT_DATE;"`                                             
exit_code=$?

if [ ${exit_code} -ne ${EXIT_CODE_SUCCESS} ]
then
   fn_log_error "Failed to fetched hive record count for table ${target_table_gold}" "${log_file_path}"
   fn_update_job_statistics "${BATCH_ID}" "${job_name}" "${start_time}" "0" "${exit_code}" "${source_date}" "${log_file_path}" "" " "   
fi 

fn_log_info "Successfully captured hive ${target_table_gold} count." "${log_file_path}"
