...
#                 1. Executes setup function to prepare work_ov_telephone_number_tbl table in work layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_OV_TELEPHONE_NUMBER_TBL}" \
     "${WORK_OV_TELEPHONE_NUMBER_TBL}" \
...
function fn_module_prepare(){
  ...
  source_table=${INCOMING_OV_TELEPHONE_NUMBER_SOURCE_TBL}
  dtm_last_updated=$1
  current_date=$2
  log_file_path=$3
  connection_url="${DRIVER}${HOST_OVCDR}:${PORT_OVCDR}:${SCHEMA_OVCDR}"
  target_dir_path="${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_OV_TELEPHONE_NUMBER_TBL}" 
  ...
  query="select /*+ NO_PARALLEL(a) */ a.*  FROM ${ORACLE_DATABASE_OVCDR}.${source_table} a WHERE a.DTM_LAST_MODIFIED >= TO_DATE('${dtm_last_updated}','yyyy-mm-dd HH24:MI:SS') AND a.DTM_LAST_MODIFIED < TO_DATE('${current_date}','yyyy-mm-dd HH24:MI:SS') AND \$CONDITIONS"
  ...
  then
     ...
     fn_log_info "Imported data from table ${source_table} to directory ${target_dir_path}" "${log_file_path}"        
  fi    
}
