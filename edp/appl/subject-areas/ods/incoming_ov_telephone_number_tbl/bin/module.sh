...
#                 1. Executes setup function to prepare incoming_ov_telephone_number_tbl table in incoming layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${INCOMING_OV_TELEPHONE_NUMBER_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_CUSTOMER}/${INCOMING_OV_TELEPHONE_NUMBER_TBL}" \
     "${INCOMING_OV_TELEPHONE_NUMBER_TBL}" \
...
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   -hivevar source_database="${HIVE_DATABASE_NAME_WORK_ODS}" \
   -hivevar target_database="${HIVE_DATABASE_NAME_INCOMING_ODS}" \
   -hivevar table_prefix="${HIVE_TABLE_PREFIX}" \
   -hivevar source_table="${WORK_OV_TELEPHONE_NUMBER_TBL}" \
   -hivevar target_table="${INCOMING_OV_TELEPHONE_NUMBER_TBL}" \
