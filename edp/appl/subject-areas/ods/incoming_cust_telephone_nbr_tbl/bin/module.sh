...
#                 1. Executes setup function to prepare incoming_cust_telephone_nbr table in incoming layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${INCOMING_CUST_TELEPHONE_NBR_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_CUSTOMER}/${INCOMING_CUST_TELEPHONE_NBR_TBL}" \
     "${INCOMING_CUST_TELEPHONE_NBR_TBL}" \
...
function fn_module_import(){
   ...
   src_table_name=`echo ${INCOMING_CUST_TELEPHONE_NBR_TBL} | cut -d'_' -f2- | tr '[:lower:]' '[:upper:]'`
   connection_url="${DRIVER}${WIFI_HOST}:${WIFI_PORT}:${WIFI_SID}"
   source_date="$1"
   log_file_path="$2"
   
   target_dir_path="${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_CUSTOMER}/${INCOMING_CUST_TELEPHONE_NBR_TBL}/source_date="${source_date}"/*"
