...
#                 1. Executes setup function to create work_bhv_acct_phone_nbr_opt table in work layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${WORK_BHV_ACCT_PHONE_NBR_OPT_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_BHV_ACCT_PHONE_NBR_OPT_TBL}" \
     "${WORK_BHV_ACCT_PHONE_NBR_OPT_TBL}" \
...
#Preparing work bhv optimum table.
function fn_module_prepare(){

   source_date=$1
   log_file_path=$2

   target_dir_path="${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_BHV_ACCT_PHONE_NBR_OPT_TBL}/*"
   ...
   pig -useHCatalog \
      ...
      -p incoming_info=${INCOMING_ACCOUNT_INFO_TBL} \
      -p incoming_tns=${INCOMING_ACCOUNT_TNS_TBL} \
      -p namenode_service=${NAMENODE_SERVICE} \
      -p default_timestamp="${DEFAULT_TIMESTAMP}" \
      -p work_hive_databse=${HIVE_DATABASE_NAME_WORK_ODS} \
      -p work_bhv_opt=${WORK_BHV_ACCT_PHONE_NBR_OPT_TBL} \
