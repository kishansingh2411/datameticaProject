...
#                 1. Executes setup function to prepare gold_rsdvr_preferences_last_rec_tbl table in gold layer.
#               		Input Arguments for this script are: Phase name (setup), Log file path(/log/file/path)                 										  
#                 2. Executes prepare function.
#               		Input Arguments for this script are: "prepare" "${source_table}" "${log_file_path}"
#				  3. Executes transform function to prepare gold_rsdvr_preferences_last_rec_tbl table in gold layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${GOLD_RSDVR_PREFERENCES_LAST_REC_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_RSDVR_PREFERENCES_LAST_REC_TBL}" \
     "${GOLD_RSDVR_PREFERENCES_LAST_REC_TBL}" \
...
#Preparing gold_rsdvr_preferences_last_rec table.
...
#Preparing gold_rsdvr_preferences_last_rec_tbl table.
