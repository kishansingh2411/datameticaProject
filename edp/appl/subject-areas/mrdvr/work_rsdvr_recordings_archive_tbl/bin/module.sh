...
#                 1. Executes setup function to prepare work_rsdvr_recordings_archive_tbl table in incoming layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${WORK_RSDVR_RECORDINGS_ARCHIVE_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA_STAGING}/${WORK_RSDVR_RECORDINGS_ARCHIVE_TBL}" \
     "${WORK_RSDVR_RECORDINGS_ARCHIVE_TBL}" \
...
#Preparing incoming_rsdvr_recordings_archive table.

function fn_module_prepare(){

  source_table="$1"
  log_file_path="$2"
  target_dir="${DATA_LAYER_DIR_WORK_SUBJECT_AREA_STAGING}/${WORK_RSDVR_RECORDINGS_ARCHIVE_TBL}/*"

  query="$(fn_build_query_for_import ${source_table} "${COL_RSDVR_RECORDINGS_ARCHIVE}")"
  ...
  #Sqoop work_rsdvr_recordings_archieve table data
