...
#                 2. Executes transform function to prepare work_rsdvr_preferences_tmp table in work layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${WORK_RSDVR_PREFERENCES_TMP_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_PREFERENCES_TMP_TBL}" \
     "${WORK_RSDVR_PREFERENCES_TMP_TBL}" \
...
#Preparing work_rsdvr_preferences_tmp table.
function fn_module_transform(){
	...
	target_dir="${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_PREFERENCES_TMP_TBL}/*"
...
beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      ...
      -hivevar work_rsdvr_preferences_tmp_tbl=${WORK_RSDVR_PREFERENCES_TMP_TBL} \
      -hivevar hive_database_name_incoming=${HIVE_DATABASE_NAME_INCOMING} \
      -hivevar incoming_rsdvr_preferences_tbl=${INCOMING_RSDVR_PREFERENCES_TBL} \
      -hivevar hive_database_name_gold=${HIVE_DATABASE_NAME_GOLD} \
      -hivevar gold_rsdvr_preferences_last_rec_tbl=${GOLD_RSDVR_PREFERENCES_LAST_REC_TBL} \
