...
#                 1. Executes setup function to prepare work_rsdvr_recordings_tmp_tbl table in gold layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_RECORDINGS_TMP_TBL}" \
     "${WORK_RSDVR_RECORDINGS_TMP_TBL}" \
...
function fn_module_transform(){
   ...
   beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      ...
      -hivevar work_rsdvr_recordings_tmp_tbl=${WORK_RSDVR_RECORDINGS_TMP_TBL} \
      -hivevar hive_database_name_incoming=${HIVE_DATABASE_NAME_INCOMING} \
      -hivevar incoming_rsdvr_recordings_tbl=${INCOMING_RSDVR_RECORDINGS_TBL} \
      ...
      -f ${hive_script_path} 1>> ${log_file_path} 2>> ${log_file_path};
      exit_code=$?
