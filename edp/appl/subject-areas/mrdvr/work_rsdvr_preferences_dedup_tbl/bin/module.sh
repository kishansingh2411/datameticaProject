...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${WORK_RSDVR_PREFERENCES_DEDUP_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_PREFERENCES_DEDUP_TBL}" \
     "${WORK_RSDVR_PREFERENCES_DEDUP_TBL}" \
...
#Preparing work_rsdvr_preferences_dedup_tbl table.
function fn_module_transform(){

	log_file_path=$1
    hive_script_path="${MODULE_HOME}/hive/module-transform.hql"
    
    target_dir="${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_PREFERENCES_DEDUP_TBL}/*"
...
beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      ...
      -hivevar work_rsdvr_preferences_seq_tbl=${WORK_RSDVR_PREFERENCES_SEQ_TBL} \
      -hivevar work_rsdvr_preferences_dedup_tbl=${WORK_RSDVR_PREFERENCES_DEDUP_TBL} \
