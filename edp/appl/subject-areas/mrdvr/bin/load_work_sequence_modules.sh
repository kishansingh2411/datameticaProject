...
then
    job_name="EDP_MRDVR_600006_WRK_PRF_SEQ"
elif [[ $table_name == "rsdvr_recordings" ]]
then
    job_name="EDP_MRDVR_600006_WRK_REC_SEQ"
elif [[ $table_name == "rsdvr_requests" ]]
then
    job_name="EDP_MRDVR_600006_WRK_REQ_SEQ"
elif [[ $table_name == "rsdvr_schedule" ]]
then
    job_name="EDP_MRDVR_600006_WRK_SCH_SEQ"
elif [[ $table_name == "rsdvr_requests_archive" ]]
then
    job_name="EDP_MRDVR_600006_WRK_RQA_SEQ"
elif [[ $table_name == "rsdvr_recordings_archive" ]]
then
    job_name="EDP_MRDVR_600006_WRK_RCA_SEQ"
else
...
target_table=${HIVE_TABLE_PREFIX}"work_${table_name}_seq"

sh ${SUBJECT_AREA_HOME}/${target_table}_tbl/bin/module.sh "transform" "${table_name}" "${log_file_path}"
exit_code=$? 

if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
then
	fn_log_error "For date ${source_date}, failed to load "${target_table}" in work layer" "${log_file_path}"
	fn_update_job_statistics "${BATCH_ID}" "${job_name}" "${start_time}" "0" "${exit_code}" "${source_date}" "${log_file_path}" "" " "
fi 

fn_log_info "For date ${source_date}, successfully loaded ${target_table} in Work layer" "${log_file_path}"
