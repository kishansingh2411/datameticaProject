...
#                 1. Executes setup function to prepare work_rsdvr_schedule_seq_tbl table in gold layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_SCHEDULE_SEQ_TBL}" \
     "${WORK_RSDVR_SCHEDULE_SEQ_TBL}" \
...
#Preparing work_rsdvr_schedule_seq_tbl table.
function fn_module_transform(){

        table_name=$1
        log_file_path=$2
        pig_script_path="${MODULE_HOME}/pig/module-transform.pig"
        target_dir="${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_SCHEDULE_SEQ_TBL}/*"
  ...
  then
    ...
    pig -useHCatalog -Ddfs.blocksize=${DFS_BLOCKSIZE} \
      ...
      -p work_rsdvr_schedule_tmp_tbl=${WORK_RSDVR_SCHEDULE_TMP_TBL} \
      -p work_rsdvr_schedule_seq_tbl=${WORK_RSDVR_SCHEDULE_SEQ_TBL} \
