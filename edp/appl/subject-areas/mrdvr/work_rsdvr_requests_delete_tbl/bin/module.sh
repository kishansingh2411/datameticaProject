...
#                 1. Executes setup function to prepare gold_rsdvr_schedule_delta_tbl table in gold layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${WORK_RSDVR_REQUESTS_DELETE_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_REQUESTS_DELETE_TBL}" \
     "${WORK_RSDVR_REQUESTS_DELETE_TBL}" \
...
#Preparing work_rsdvr_schedule_delete_tbl table.
function fn_module_transform(){
	...
	target_dir="${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_REQUESTS_DELETE_TBL}/*"
...
beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      ...
      -hivevar work_rsdvr_requests_delete_tbl=${WORK_RSDVR_REQUESTS_DELETE_TBL} \
      -hivevar work_rsdvr_requests_tmp_tbl=${WORK_RSDVR_REQUESTS_TMP_TBL} \
