...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_REQUESTS_DEDUP_TBL}" \
     "${WORK_RSDVR_REQUESTS_DEDUP_TBL}" \
...
#Preparing work_rsdvr_requests_dedup_tbl table.
...
beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      ...
      -hivevar work_rsdvr_requests_seq_tbl=${WORK_RSDVR_REQUESTS_SEQ_TBL} \
