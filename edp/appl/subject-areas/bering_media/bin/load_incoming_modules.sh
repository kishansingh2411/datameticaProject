...
#   Usage       : load_incoming_modules.sh "201510150100" 
#   Date        : 12/28/2015
#   Log File    : .../log/bering_media/BERING_MEDIA_MOVE_TO_INCOMING_JOB.log
#   SQL File    : 
#   Error File  : .../log/bering_media/BERING_MEDIA_MOVE_TO_INCOMING_JOB.log
#   Dependency  : 
#   Disclaimer  : 
#
...
job_name="BERING_MEDIA_MOVE_TO_INCOMING_JOB"
...
do
   ...
   sh $SUBJECT_AREA_HOME/${table}_tbl/bin/module.sh "transform" "${source_file_path}" \
      "${source_timestamp}" "${log_file_path}"
   exit_code=$?   

   if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
   then
      fn_log_error "Failed to transformed "${table_name}" in Incoming layer" "${log_file_path}"
