...
#                 1. Executes setup function to prepare incoming_nz_t_f_split_channel_tuning_rst table in incoming layer.
...
function fn_module_prepare(){   
  ...
  fn_distcp_from_s3 \
  	...
  	"${NUM_MAPPERS}" \
  	"${source_dir}" \
  	"${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_NZ_T_F_SPLIT_CHANNEL_TUNING_RST_TBL}/broadcast_month=${load_month}"
  exit_code=$?
  
  if [ ${exit_code} -ne ${EXIT_CODE_SUCCESS} ]
  then
  	 fn_log_info "Failed to copy data from S3 to HDFS for table ${table_name} " "${log_file_path}"
     return $exit_code
  fi
  
  fn_alter_table_with_one_partitions "${HIVE_DATABASE_NAME_INCOMING}" \
	"${HIVE_TABLE_PREFIX}${INCOMING_NZ_T_F_SPLIT_CHANNEL_TUNING_RST_TBL}" \
