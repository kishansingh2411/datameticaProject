...
#                 1. Executes setup function to prepare work_t_cvb_chdv_a_hh_series_summary_tbl table in incoming layer.
...
#				: Prepare - 
#						sh module.sh "Prepare" "${max_seq_id}" "${source_table}" "/local/file/path" ${start_date} ${end_date}
#   Date        : 05/05/2016
#   Log File    : 
#   SQL File    : 
#   Error File  : 
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${WORK_T_CVB_CHDV_A_HH_SERIES_SUMMARY_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_T_CVB_CHDV_A_HH_SERIES_SUMMARY_TBL}" \
     "${WORK_T_CVB_CHDV_A_HH_SERIES_SUMMARY_TBL}" \
...
#Preparing work_t_cvb_chdv_a_hh_series_summary_tbl table.
...
function fn_module_prepare(){   
  ...
  target_dir="${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_T_CVB_CHDV_A_HH_SERIES_SUMMARY_TBL}"
  start_date="$3"
  end_date="$4"
 ...
 #Sqoop EDP_T_CVB_CHDV_A_HH_SERIES_SUMMARY table data
  ...
  sh ${MODULE_HOME}/bin/module.sh setup ${log_file_path}
   ...
   if [ ${exit_code} -ne ${EXIT_CODE_SUCCESS} ]
   then
      fn_log_error "Failed while recreating the table ${WORK_T_CVB_CHDV_A_HH_SERIES_SUMMARY_TBL}" "${log_file_path}"
      return -1
   fi
