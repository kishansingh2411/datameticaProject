...
#   Purpose:    : Functions to prepare gold_item_master table in Gold layer
#                 1. Executes setup function to create gold_item_master table in Gold Layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA_LOOKUP}" \
    "${GOLD_ITEM_MASTER_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA_LOOKUP}/${GOLD_ITEM_MASTER_TBL}" \
     "${GOLD_ITEM_MASTER_TBL}" \
...
#Transforming search_engines table at incoming layer
