...
job_name="$(fn_generate_job_name "$key_param_id" "$suite_name" "LD_INCOM_MDULS")"
...
##############################################################################
   ...
   record_count=`beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} --showHeader=false --silent=true --outputformat=tsv2 -e \
                 "SELECT COUNT(*) FROM $HIVE_DATABASE_NAME_INCOMING.${HIVE_TABLE_PREFIX}${INCOMING_HIT_DATA_TBL} \
