...
#   Purpose:    : Functions to prepare Optimum_link table in Incoming layer
#                 1. Executes setup function to create Optimum_link table in Incoming Layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_LOOKUP}/${INCOMING_OPTIMUM_LINK_TBL}" \
     "${INCOMING_OPTIMUM_LINK_TBL}" \
...
#Preparing Optimum_Link table at incoming layer
function fn_module_prepare(){
  ...
  fn_copy_from_local \
     "${SUBJECT_AREA_HOME}/metadata/Optimum_Link.txt" \
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_LOOKUP}/${INCOMING_OPTIMUM_LINK_TBL}" \
     "${BOOLEAN_FALSE}" \
     "${BOOLEAN_FALSE}" \
     "${log_file_path}"
