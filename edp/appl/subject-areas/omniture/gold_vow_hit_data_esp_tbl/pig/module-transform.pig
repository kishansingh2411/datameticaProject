...
--#   Purpose:    : Loading data from incoming_hit_data table to gold_vow_hit_data_esp table
...
gold_h_optimum_user = 
	LOAD '${hive_database_name_gold}.${gold_h_optimum_user_tbl}' 
    USING org.apache.hive.hcatalog.pig.HCatLoader()
;	
...
--Filter hit_date by suite_name an source_date
incoming_hit_data_filter = 
	FILTER incoming_hit_data 
    BY (suite_name=='$suite_name' AND source_date == '$source_date' )
;
							
incoming_hit_data_filter_with_condtion = 
	FILTER incoming_hit_data_filter 
    BY (post_page_event == 0 
        AND hit_source  != 5 
        AND hit_source  != 8 
        AND hit_source  != 9 
        AND exclude_hit <= 0
...
gold_h_optimum_user_generated = 
	FOREACH gold_h_optimum_user_active 
...
SPLIT incoming_hit_data_filter_with_condtion 
	INTO hit_data_with_mandatory_records
   	IF (
   		browser is not null  
   		AND connection_type is not null  
   		AND date_time is not null 
   		AND TRIM(domain) is not null 
   		AND TRIM(domain) !='' 
        AND mobile_id is not null 
        AND os is not null  
        AND TRIM(page_url) is not null 
        AND TRIM(page_url) !=''  
        AND TRIM(post_visid_high) is not null 
        AND TRIM(post_visid_high) !='' 
        AND TRIM(post_visid_low) is not null 
        AND TRIM(post_visid_low) !='' 
...
hit_data_decrypted_records = 
	...
	GENERATE 
		encryptDecrypt('$suite_name', 'OPTIMUM_ID','$username', evar11, 'DECRYPT') AS evar11_optimum_id,
		encryptDecrypt('$suite_name', 'EMAIL','$username', evar14, 'DECRYPT')      AS evar14_email,
		encryptDecrypt('$suite_name', 'TEL_NBR','$username', evar16, 'DECRYPT')    AS evar16_tel_nbr,
		*
;
...
SPLIT hit_data_decrypted_records 
    ...
    IF (
    	TRIM(evar11_optimum_id) is null 
    	OR TRIM(evar11_optimum_id) == ''
    	),
	hit_data_decrypted_records_with_optimumId OTHERWISE;

hit_data_records_missing_optimumId_generated = 
	FOREACH hit_data_decrypted_records_missing_optimumId 
	GENERATE
        -1 as corp,
		-1 as cust,
		-1 as dwelling_nbr,
		...
		..duplicate_purchase,
		evar1  AS evar1_pagename,
		evar2..
;
								 
hit_data_decrypted_records_with_optimumId_join = 
	JOIN hit_data_decrypted_records_with_optimumId  BY evar11_optimum_id LEFT OUTER, 
...
hit_data_records_with_optimumId_generated = 
	...
	GENERATE 
		...
		(TRIM(gold_h_optimum_user_generated::cust) == '' OR gold_h_optimum_user_generated::cust is null ? '-1' : gold_h_optimum_user_generated::cust) AS cust,
 		(TRIM(gold_h_optimum_user_generated::dwelling_nbr) == '' OR gold_h_optimum_user_generated::dwelling_nbr is null ? '-1' : gold_h_optimum_user_generated::dwelling_nbr) AS dwelling_nbr,
  		(gold_h_optimum_user_generated::optimum_user_id is null ? -1 : gold_h_optimum_user_generated::optimum_user_id) AS optimum_user_id,
		evar11_optimum_id       AS evar11_optimum_id,
		evar14_email            AS evar14_email,
		evar16_tel_nbr          AS evar16_tel_nbr,
...
hit_data_records = 
	UNION ONSCHEMA hit_data_records_missing_optimumId_generated, 
	hit_data_records_with_optimumId_generated
;
...
hit_data_records_join_pagenames = 
	JOIN hit_data_records_join_search_engines BY pagename LEFT,
...
hit_data_records_join_isp = 
    JOIN hit_data_records_join_site_section BY domain LEFT,
	incoming_optimum_isp_filter BY isp_name
	USING 'replicated'
; 
					
incoming_plugins_tuples = foreach incoming_plugins_filter generate TOTUPLE(*) as (plugin_recs:tuple());

incoming_plugins_tbl_filter_all = group incoming_plugins_tuples all;

incoming_plugins_generate = foreach incoming_plugins_tbl_filter_all generate TOBAG(*) as plugin_rec_bag;

hit_data_records_join_plugins = foreach hit_data_records_join_isp generate ..plugins, ResolvePluginId(plugins, incoming_plugins_generate.plugin_rec_bag) as plugins_name, post_browser_height..;
...
generated_records = 
	...
	GENERATE 
		...
		cust_visid              AS cust_visid,              
		daily_visitor           AS daily_visitor,           		    
        ToString(CurrentTime()) AS  dtm_created,			
		duplicate_events        AS duplicate_events,        		         	
		duplicate_purchase      AS duplicate_purchase,
		duplicated_from         AS duplicated_from,
		(chararray)dwelling_nbr AS dwelling_nbr,
        hit_data_records_join_site_section::hit_data_records_join_pagenames::hit_data_records_join_search_engines::hit_data_records_join_referrer_type::hit_data_records_join_operating_systems::hit_data_records_join_languages::hit_data_records_join_javascript_version::hit_data_records_join_country::hit_data_records_join_connection_type::hit_data_records_join_browser::hit_data_records::evar1_pagename          AS evar1_pagename,        
		evar10         			AS evar10,
		evar11_optimum_id       AS evar11_optimum_id,
		evar12         			AS evar12,
		evar13         			AS evar13,
		...
		evar74         			AS evar74,
		evar75         			AS evar75,
		evar8         			AS evar8_custom_link_tracking,
		evar9         			AS evar9,
		event_list         		AS event_list,
		exclude_hit         	AS exclude_hit,
		first_hit_page_url      AS first_hit_page_url,
		...
		ip         				AS ip,          			
		ip2         			AS ip2,
		incoming_optimum_isp_filter::isp_id  as   isp_id,
        domain                  AS isp_name,
		j_jscript         		AS j_jscript, 		
		java_enabled         	AS java_enabled,
		...
		hit_data_records_join_site_section::hit_data_records_join_pagenames::hit_data_records_join_search_engines::hit_data_records_join_referrer_type::hit_data_records_join_operating_systems::hit_data_records_join_languages::incoming_languages_filter::language_name AS language_name,
		last_hit_time_gmt       AS last_hit_time_gmt,       
		last_purchase_num       AS last_purchase_num,       
		last_purchase_time_gmt  AS last_purchase_time_gmt, 
		mobile_id         		AS mobile_id,
	    ...
	    hit_data_records_join_site_section::hit_data_records_join_pagenames::hit_data_records_join_search_engines::hit_data_records_join_referrer_type::hit_data_records_join_operating_systems::incoming_operating_systems_filter::operating_system_name AS operating_system_name,
		optimum_user_id      	AS optimum_user_id,
		p_plugins         		AS p_plugins,
		page_event         		AS page_event,          	
		page_event_var1         AS page_event_var1,         
		page_event_var2         AS page_event_var2,         
		page_event_var3         AS page_event_var3,         
        hit_data_records_join_site_section::hit_data_records_join_pagenames::incoming_optimum_pagenames_filter::page_name_id   AS   page_name_id ,		
		page_type         		AS page_type,
		page_url         		AS page_url, 
		...
		ref_domain	            AS ref_domain,	            
		referrer                AS referrer,                
		hit_data_records_join_site_section::hit_data_records_join_pagenames::hit_data_records_join_search_engines::hit_data_records_join_referrer_type::incoming_referrer_type_filter::referrer_type_fullname AS   referrer_type_fullname,
		ref_type         		AS referrer_type_id,  
		hit_data_records_join_site_section::hit_data_records_join_pagenames::hit_data_records_join_search_engines::hit_data_records_join_referrer_type::incoming_referrer_type_filter::referrer_type_shortname  AS referrer_type_shortname  ,
		resolution         		AS resolution,      	
		s_resolution         	AS s_resolution, 
		sampled_hit         	AS sampled_hit,        
		search_engine         	AS search_engine_id,
        hit_data_records_join_site_section::hit_data_records_join_pagenames::hit_data_records_join_search_engines::incoming_search_engines_filter::search_engine_name AS search_engine_name ,		
		search_page_num	   		AS search_page_num,			
		secondary_hit  			AS secondary_hit,
		service         		AS service,         
		hit_data_records_join_site_section::incoming_optimum_site_section_filter::site_section_id   AS   site_section_id,
...
STORE generated_records
   INTO '${hive_database_name_gold}.${gold_vow_hit_data_esp_tbl}'
