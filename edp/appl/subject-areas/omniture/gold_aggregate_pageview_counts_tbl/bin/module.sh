...
#   Purpose:    : Module script to calculate gold aggregate pageview count
#                 1. Executes setup function to prepare directory structure for the table											                                						  
#                    and create gold_aggeregate_pageview_counts_tbl in gold layer.
...
#				: Transform - 
#						sh module.sh "transform" "${suite_name}" "2015-12-28" "2015-12-28" "daily" \
#                       "20151228" "/log/file/path"
#
#   Date        : 12/28/2015
#   Log File    : 
#   SQL File    : 
...
#Create module data directory if not exists.
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${SMITH_AGGREGATE_PAGEVIEW_COUNTS_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
 ...
 # Create hive table
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_AGGREGATE_PAGEVIEW_COUNTS_TBL}" \
     "${GOLD_AGGREGATE_PAGEVIEW_COUNTS_TBL}" \
 ...
 # Create hive table
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_SMITH_SUBJECT_AREA_AGGREGATE}/${SMITH_AGGREGATE_PAGEVIEW_COUNTS_TBL}" \
     "${SMITH_AGGREGATE_PAGEVIEW_COUNTS_TBL}" \
     "${MODULE_HOME}/schema/module.hql" \
     "${log_file_path}" \
     "${HIVESERVER2_URL}" \
...
# Deriving aggregate pagview count table at Gold layer
function fn_module_transform(){
   ...
   then
      fn_log_info "Executing for cablevis-uow-com" "${log_file_path}"
      table_name=${GOLD_UNIFIED_HIT_DATA_TBL}
                  
   else
      fn_log_error "Got invalid suite_name $suite_name while transform phase of $frequency pageview counts aggregate"  \
         ${log_file_path}
      return -1
   fi
   ...
   fn_calculate_aggregates \
      ...
      "${HIVE_TABLE_PREFIX}${GOLD_AGGREGATE_PAGEVIEW_COUNTS_TBL}" \
      "${HIVE_TABLE_PREFIX}${SMITH_AGGREGATE_PAGEVIEW_COUNTS_TBL}" \
      "$hive_script_path" \
      "${log_file_path}" \
      "$HIVESERVER2_URL" \
      "${HIVE_USENAME}" \
      ...
      then
         fn_log_error "Failed to calculate $frequency PageView Counts aggregates for gold layer, Quitting the process" "${log_file_path}"
	  	 return $exit_code
	  fi
}
###############################################################################
