...
#   Purpose:    : Functions to prepare Optimum_event_list table in Incoming layer
#                 1. Executes setup function to create Optimum_event_list table in Incoming Layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_LOOKUP}/${INCOMING_OPTIMUM_EVENT_LIST_TBL}" \
     "${INCOMING_OPTIMUM_EVENT_LIST_TBL}" \
 ...
 #Preparing Optimum_event_list table at incoming layer 
  ...
  fn_copy_from_local \
     "${SUBJECT_AREA_HOME}/metadata/Optimum_Event_List.txt" \
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_LOOKUP}/${INCOMING_OPTIMUM_EVENT_LIST_TBL}" \
     "${BOOLEAN_FALSE}" \
     "${BOOLEAN_FALSE}" \
     "${log_file_path}"
