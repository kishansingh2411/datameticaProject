...
#   Purpose:    : Module script to load gold opt esp hit data table
#                 1. Executes setup function to prepare directory structure for the table											                                						  
#                    and create gold_opt_hit_data_esp_tbl in gold layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_OPT_HIT_DATA_ESP_TBL}" \
     "${GOLD_OPT_HIT_DATA_ESP_TBL}" \
...
function fn_module_transform(){
    ...
    pig -useHCatalog -p suite_name=$suite_name \
       ...
       -p incoming_optimum_error_message_tbl="${HIVE_TABLE_PREFIX}${INCOMING_OPTIMUM_ERROR_MESSAGE_TBL}" \
       -p gold_opt_hit_data_esp_tbl="${HIVE_TABLE_PREFIX}${GOLD_OPT_HIT_DATA_ESP_TBL}" \
