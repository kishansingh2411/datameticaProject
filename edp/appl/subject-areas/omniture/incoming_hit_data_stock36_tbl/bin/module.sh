...
#                 1. Executes setup function to create Stock36 Hit Data table in Incoming Layer.
...
#                                                         Suite Id (8), Source date (2015-12-28),
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_HIT_DATA_STOCK36_TBL}" \
     "${INCOMING_HIT_DATA_STOCK36_TBL}" \
...
function fn_module_transform(){
    ...
    pig -useHCatalog -p suite_name="${suite_name}" \
		...
		-p incoming_hit_data_stock36_tbl="${HIVE_TABLE_PREFIX}${INCOMING_HIT_DATA_STOCK36_TBL}" \
		-p incoming_hit_data_stock36_raw_tbl="${HIVE_TABLE_PREFIX}${INCOMING_HIT_DATA_STOCK36_RAW_TBL}" \
