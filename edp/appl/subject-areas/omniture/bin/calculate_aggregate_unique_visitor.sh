...
#   Department: AM
#
#   Program name: calculate_aggregate_unique_visitor.sh
...
if [ "$#" -ne 2 ]; then
  echo "Illegal number of parameters.Parameters expected [key_param_id frequency(daily/weekly/monthly/quarterly/yearly) ]"
  exit 
fi
...
then
	case "$frequency" in
	   "daily")
	      job_name="UOW_600002_DLY_AGGR_UNQ_VSTR"
	      ;;
	   "weekly")
	      job_name="UOW_600002_WKLY_AGGR_UNQ_VSTR"
	      ;;
	   "monthly")
	      job_name="UOW_600002_MNTH_AGGR_UNQ_VSTR"
	      ;;
	   "quarterly")
	      job_name="UOW_600002_QTR_AGGR_UNQ_VSTR"
	      ;;
	   "yearly")
	      job_name="UOW_600002_YRLY_AGGR_UNQ_VSTR"
	      ;;
	esac
...
then
  	case "$frequency" in
	   "daily")
	      job_name="ONET_600009_DLY_AGGR_UNQ_VSTR"
	      ;;
	   "weekly")
	      job_name="ONET_600009_WKLY_AGGR_UNQ_VSTR"
	      ;;
	   "monthly")
	      job_name="ONET_600009_MNTH_AGGR_UNQ_VSTR"
	      ;;
	   "quarterly")
	      job_name="ONET_600009_QTR_AGGR_UNQ_VSTR"
	      ;;
	   "yearly")
	      job_name="ONET_600009_YRLY_AGGR_UNQ_VSTR"
	      ;;
	esac
else
...
##############################################################################
#                                                                            #
# Calculating Unique Visitor aggregates for provided frequency               #
#                                                                            #
##############################################################################
...
sh $SUBJECT_AREA_HOME/gold_aggregate_unique_visitor_count_tbl/bin/module.sh "transform" "${suite_name}" \
   "${period_start_date}" "${period_end_date}" "${frequency}" "${duration}" "${log_file_path}"
exit_code="$?"

if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
then
	fn_log_error "Failed to calculate $frequency unique visitor aggregates for suite $suite_name, Quitting the process" "${log_file_path}"	
	fn_update_job_statistics "${batch_id}" "${job_name}" "${start_time}" "NA" "${exit_code}" \
	  "${source_date}" "${log_file_path}" "${source_file_name}" "$args"
fi

fn_log_info "Successfully computed $frequency unique visitor aggregates for gold layer " "${log_file_path}"
