...
then
	job_name="UOW_600002_VDATE_MV_TO_INCOM"
elif [ $suite_name == "$CHANNEL900" ]
then
   	job_name="STK36_600005_VDAT_MV_TO_INCOM"
elif [ $suite_name == "$ONET_PROD" ]
then
   	job_name="ONET_600009_VDATE_MV_TO_INCOM"
else
	      ...
	      tsv_fileName="$(basename $tsv_file | cut -d"." -f1)"	  
	        ...
	        then
	            ...
	            then	        	        
	                ...
	                then
	                	  fn_hadoop_delete_file "$DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_STAGING/incoming_"${tsv_fileName}"_stock36_raw/*" "${BOOLEAN_FALSE}" "${log_file_path}"
	                	  exit_code=$?    
	    
	                      if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
	    		          then
	    		              fn_log_error "For suite $suite_name, failed to delete stock36 raw table data, Quitting the prossess..!!!" "${log_file_path}"
	    	     		      fn_update_job_statistics "${batch_id}" "${job_name}" "${start_time}" "NA" "${EXIT_CODE_FAIL}" \
	  	   			             "${source_date}" "${log_file_path}" "${source_file_name}" " "
		   			      fi   
		   			      
	                      fn_hadoop_move_file_or_directory "$tsv_file" \
	                      "$DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_STAGING/incoming_"${tsv_fileName}"_stock36_raw/" \
	                      "${BOOLEAN_FLASE}" "${BOOLEAN_FALSE}" "${log_file_path}"
	                      exit_code=$?    
	    
	                      if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
	    		          then
	    		              fn_log_error "For suite $suite_name, failed to move stock36 data file into incoming hit data stock36 table, Quitting the prossess..!!!" "${log_file_path}"
	    	     		      fn_update_job_statistics "${batch_id}" "${job_name}" "${start_time}" "NA" "${EXIT_CODE_FAIL}" "${source_date}" "${log_file_path}" "${source_file_name}" " "
		   			      fi                                               
                    else             
	                      fn_hadoop_move_file_or_directory "$tsv_file" \
	                      "$DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA/incoming_$tsv_fileName/suite_name=${suite_name}/source_date=${source_date}/" \
	                      "${BOOLEAN_FLASE}" "${BOOLEAN_FALSE}" "${log_file_path}"
	                      exit_code=$?    
	    
	    		          if [ ${exit_code} -ne $EXIT_CODE_SUCCESS ]
	    		          then
	    		              fn_log_error "For suite $suite_name, failed to move hit data file into incoming hit data table, Quitting the prossess..!!!" "${log_file_path}"
	  	     		          fn_update_job_statistics "${batch_id}" "${job_name}" "${start_time}" "NA" "${EXIT_CODE_FAIL}" "${source_date}" "${log_file_path}" "${source_file_name}" " "
	    		          fi   
                    fi      
	            else
