...
#                    and create work_a_int_rad_call_usage_tbl in work layer.
...
#   ------   --------------------     ----------       -------------------------------------
#    1.0     DataMetica Team          12/16/2016      Initial version
#
#
#####################################################################################################################
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_A_INT_RAD_CALL_USAGE_TBL}" \
     "${WORK_A_INT_RAD_CALL_USAGE_TBL}" \
...
# Deriving a_int_rad_call_usage_tbl table at work layer
...
function fn_module_import(){
  ...
  then
   ...
   fn_hadoop_delete_directory_if_exists "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_A_INT_RAD_CALL_USAGE_TBL}/*" \
     "${BOOLEAN_FALSE}" \
     "${log_file_path}"
   exit_code=$?
...
function fn_module_prepare(){
  ...
  if [ ${exit_code} -ne ${EXIT_CODE_SUCCESS} ]
  then
      fn_log_error "Failed to delete directory "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_A_INT_RAD_CALL_USAGE_TBL}/*" " "${log_file_path}"
	  return $exit_code
  fi
  fn_log_info "Successfully deleted directory "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_A_INT_RAD_CALL_USAGE_TBL}/*" " "${log_file_path}"	
 ...
 fn_execute_hive \
    ...
    ${GOLD_METADATA_INT_RAD_CALL_USAGE_TBL} \
    ${file_name} \
    ${hql_file} \
    ${log_file_path}
    exit_code=$?
