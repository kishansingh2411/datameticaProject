...
FROM(
   SELECT 
      ...
      CASE 
         WHEN (LTRIM(RTRIM(INCOMING.CALL_TYPE))) IS NULL THEN 'NULL' 
         ELSE  UPPER(INCOMING.CALL_TYPE) 
      END AS CALL_TYPE,
      CASE 
         WHEN INCOMING.SECONDS_OF_USE IS NULL THEN 0 
         ELSE INCOMING.SECONDS_OF_USE 
      END  AS SECONDS_OF_USE,
      CASE 
         WHEN INCOMING.NUMBER_OF_CALLS IS NULL THEN 0 
         ELSE INCOMING.NUMBER_OF_CALLS 
      END  AS NUMBER_OF_CALLS,
      INCOMING.BATCH_ID AS BATCH_NBR,
      CASE 
         WHEN INCOMING.ATTEMPT_COUNT IS NULL THEN 0 
         ELSE INCOMING.ATTEMPT_COUNT 
      END  AS ATTEMPT_COUNT,
      CASE 
         WHEN WORK.CORP IS NULL THEN '-1' 
         ELSE WORK.CORP  
      END AS CORP,
      CASE 
         WHEN WORK.HOUSE IS NULL THEN '-1' 
         ELSE WORK.HOUSE  
      END AS HOUSE,
      CASE 
         WHEN WORK.CUST IS NULL THEN '-1' 
         ELSE WORK.CUST 
      END AS CUST,
