...
#                    and create work_a_dom_nsn_out_usage_tbl in work layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_A_DOM_NSN_OUT_USAGE_TBL}" \
     "${WORK_A_DOM_NSN_OUT_USAGE_TBL}" \
...
# Deriving a_dom_out_usage_tbl table at work layer
...
function fn_module_import(){
 ...
 query="$(fn_build_query_for_import ${source_table} ${ORACLE_DATABASE})"
  ...
  then
   ...
   fn_hadoop_delete_directory_if_exists "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_A_DOM_NSN_OUT_USAGE_TBL}/*" \
     "${BOOLEAN_FALSE}" \
     "${log_file_path}"
    exit_code=$?
    if [ ${exit_code} -ne ${EXIT_CODE_SUCCESS} ]
    then
        fn_log_error "Failed to delete directory "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_A_DOM_NSN_OUT_USAGE_TBL}/*" " "${log_file_path}"
	    return $exit_code
    fi
    fn_log_info "Successfully deleted directory "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_A_DOM_NSN_OUT_USAGE_TBL}/*" " "${log_file_path}"
  
  fn_log_info "Successfully deleted directory "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_A_DOM_NSN_IN_USAGE_TBL}/*" " "${log_file_path}"
...
function fn_module_prepare(){
  ...
  fn_execute_hive \
    ...
    ${GOLD_METADATA_DOM_NSN_OUT_USAGE_TBL} \
    ${file_name} \
    ${hql_file} \
    ${log_file_path}
    exit_code=$?
