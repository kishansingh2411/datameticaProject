...
function fn_execute_hive_chc(){
   ...
   beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      ...
      -hivevar incoming_agg_data=${incoming_agg_data} \
      -hivevar source_date=${source_date} \
      -hivevar hive_database_name_gold=${HIVE_DATABASE_NAME_GOLD} \
      -hivevar gold_agg_data=${gold_agg_data} \
      -hivevar hive_database_name_ods_incoming=${HIVE_DATABASE_NAME_INCOMING_ODS} \
      -hivevar ov_telephone_number=${INCOMING_OV_TELEPHONE_NUMBER_TBL} \
      -hivevar customer_account=${INCOMING_CUSTOMER_ACCOUNT_TBL} \
      -hivevar ip_sip_did_phone_nbr=${INCOMING_IP_SIP_DID_PHONE_NBR_TBL} \
      -hivevar ip_sip_pilot_phone_nbr=${INCOMING_IP_SIP_PILOT_PHONE_NBR_TBL} \
      -hivevar hive_database_name_work=${HIVE_DATABASE_NAME_WORK} \
      -hivevar work_agg_chc=${work_agg_chc} \
      -hivevar param_numeric=${param_numeric} \
      -f ${hql_script_path} 1>> ${log_file_path} 2>> ${log_file_path};
   exit_code=$?
...
function fn_execute_hive(){
 ...
 beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar hive_database_name_ods_gold=${HIVE_DATABASE_NAME_GOLD_ODS} \
   -hivevar gold_bhv_opt_sdl="${GOLD_BHV_ACCT_PHONE_NBR_OPT_SDL_TBL}" \
