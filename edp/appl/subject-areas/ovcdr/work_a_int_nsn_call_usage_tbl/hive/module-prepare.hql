...
FROM(
   SELECT 
      INCOMING.TELEPHONE_NUMBER AS TELEPHONE_NUMBER,
      CASE WHEN INCOMING.PRODUCT_NAME IS NULL THEN 'NULL' ELSE UPPER(INCOMING.PRODUCT_NAME) END  AS PRODUCT_NAME,
      CASE WHEN INCOMING.PLAN_NUMBER IS NULL OR TRIM(INCOMING.PLAN_NUMBER) = '' THEN '-1' ELSE UPPER(INCOMING.PLAN_NUMBER)  END  AS PLAN_NUMBER,
      ...
      INCOMING.USAGE_DATE AS USAGE_DATE
