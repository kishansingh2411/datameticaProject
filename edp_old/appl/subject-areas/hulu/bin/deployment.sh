...
log_file_path=$LOG_DIR_SUBJECT_AREA/HULU_DEPLOYMENT.log
...
`psql -h $POSTGRES_HOST -p $POSTGRES_PORT -U $POSTGRES_USERNAME -d $POSTGRES_DATABASE_NAME \
   -a -f $PROJECT_HOME/rdbms_schema/postgres_schema.sql` 1>> ${log_file_path} 2>> ${log_file_path}
exit_code=$?
