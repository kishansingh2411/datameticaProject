...
#                    and create incoming_code999_tbl in incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_CODE999_TBL}" \
     "${INCOMING_CODE999_TBL}" \
...
#Preparing incoming EDP_CODE999_TBL table.
function fn_module_prepare(){   
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar source_table="${WORK_CODE999_TBL}" \
   -hivevar target_table="${INCOMING_CODE999_TBL}" \
