...
#                 1. Executes setup function to prepare smith_t_d_res_demographic_tbl table in smith layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_SMITH_SUBJECT_AREA_DIMENSION}/${SMITH_T_D_RES_DEMOGRAPHIC_TBL}" \
     "${SMITH_T_D_RES_DEMOGRAPHIC_TBL}" \
...
#Preparing smith_t_d_res_demographic_tbl table.
function fn_module_prepare(){   
  ...
  hadoop fs -test -e ${DATA_LAYER_DIR_SMITH_SUBJECT_AREA_DIMENSION}/${SMITH_T_D_RES_DEMOGRAPHIC_TBL}
  exit_code=$?
  
  if [ ${exit_code} == 0 ]
  then
     hadoop fs -rm -r "${DATA_LAYER_DIR_SMITH_SUBJECT_AREA_DIMENSION}/${SMITH_T_D_RES_DEMOGRAPHIC_TBL}/*"
  fi
