...
#                 1. Executes setup function to prepare gold_rsdvr_preferences_delta_tbl table in gold layer.
#               		Input Arguments for this script are: Phase name (setup), Log file path(/log/file/path)                 										  
#                 2. Executes prepare function.
#               		Input Arguments for this script are: "prepare" "${source_table}" "${log_file_path}"
#				  3. Executes transform function to prepare gold_rsdvr_preferences_delta_tbl table in gold layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${GOLD_RSDVR_PREFERENCES_DELTA_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_RSDVR_PREFERENCES_DELTA_TBL}" \
     "${GOLD_RSDVR_PREFERENCES_DELTA_TBL}" \
...
#Preparing gold_rsdvr_preferences_delta table.
...
function fn_module_import(){
  ...
  query="$(fn_build_query_for_import_from_netezza t_${source_table} "${COL_RSDVR_PREFERENCES_DELTA}")"
...
beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      ...
      -hivevar gold_rsdvr_preferences_delta=${GOLD_RSDVR_PREFERENCES_DELTA_TBL} \
      -hivevar hive_database_name_work=${HIVE_DATABASE_NAME_WORK} \
      -hivevar work_rsdvr_preferences_dedup=${WORK_RSDVR_PREFERENCES_DEDUP_TBL} \
...
#Preparing gold_rsdvr_preferences_delta_tbl table.
...
beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      -hivevar hive_database_name_gold=${HIVE_DATABASE_NAME_GOLD} \
      -hivevar gold_rsdvr_preferences_delta=${GOLD_RSDVR_PREFERENCES_DELTA_TBL} \
      -hivevar gold_rsdvr_preferences_last_rec=${GOLD_RSDVR_PREFERENCES_LAST_REC_TBL} \
      -hivevar hive_database_name_work=${HIVE_DATABASE_NAME_WORK} \
      -hivevar work_rsdvr_preferences_delete_dedup=${WORK_RSDVR_PREFERENCES_DELETE_DEDUP_TBL} \
