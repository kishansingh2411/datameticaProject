...
# Deriving work_a_vma_nsn_call_usage_tbl table at gold layer
...
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar incoming_table="${INCOMING_STG_VMA_NSN_CALL_TBL}" \
   -hivevar gold_table="${GOLD_A_VMA_NSN_CALL_USAGE_TBL}" \
   -hivevar work_chc_table="${WORK_CHC_VMA_NSN_TBL}" \
