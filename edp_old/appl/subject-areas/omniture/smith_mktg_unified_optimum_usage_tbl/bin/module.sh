...
#                    and create smith_mktg_unified_optimum_usage_tbl in smith layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    "${DATA_LAYER_DIR_SMITH_SUBJECT_AREA_MKT_FACT}" \
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_SMITH_SUBJECT_AREA_MKT_FACT}/${SMITH_MKTG_UNIFIED_OPTIMUM_USAGE_TBL}" \
     "${SMITH_MKTG_UNIFIED_OPTIMUM_USAGE_TBL}" \
...
# Deriving unified optimum usage table at Smith layer
fn_module_transform(){
   ...
   fn_load_smith_fact_tables \
         ...
         "${HIVE_TABLE_PREFIX}${GOLD_UNIFIED_HIT_DATA_TBL}" \
         "${HIVE_DATABASE_NAME_SMITH}" \
         "${HIVE_TABLE_PREFIX}${SMITH_MKTG_UNIFIED_OPTIMUM_USAGE_TBL}" \
         "${hive_script_path}" \
         "${log_file_path}" \
         "${HIVESERVER2_URL}" \
         "${HIVE_USENAME}" \
...
#      -hivevar gold_unified_hit_data_tbl="${HIVE_TABLE_PREFIX}${GOLD_UNIFIED_HIT_DATA_TBL}" \
#      -hivevar smith_mktg_unified_optimum_usage=${SMITH_MKTG_UNIFIED_OPTIMUM_USAGE_TBL} \
