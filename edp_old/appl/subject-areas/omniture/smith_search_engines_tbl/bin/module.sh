...
#                    and create smith_search_engines_tbl in smith layer.
#               		Input Arguments for this script are: Phase name (setup), Log file path(/log/file/path)                 										  
#                 2. Executes transform function.
#               		Input Arguments for this script are: Phase name (transform), suite Id(7), Log file path(/local/file/path)  
#															 
#   Author(s)   : DataMetica Team
#   Usage       : setup - 
#						sh module.sh "setup" "/log/file/path" 
#				: transform - 
#						sh module.sh "transform" "7" "/log/file/path"
#   Date        : 12/28/2015
#   Log File    : 
#   SQL File    : 
#   Error File  : 
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_SMITH_SUBJECT_AREA_DIMENSION}/${SMITH_SEARCH_ENGINES_TBL}" \
     "${SMITH_SEARCH_ENGINES_TBL}" \
...
# Deriving search engines table at Smith layer
...
# 		-hivevar dtm_created="$dtm_created" \
 #       -hivevar hive_database_name_smith="${HIVE_DATABASE_NAME_SMITH}" \
	#	-hivevar smith_search_engines_tbl="${SMITH_SEARCH_ENGINES_TBL}" \
	#	-hivevar hive_database_name_incoming="${HIVE_DATABASE_NAME_INCOMING}" \
	#	-hivevar incoming_search_engines_tbl="${INCOMING_SEARCH_ENGINES_TBL}" \
