...
#                    and create gold_stock36_hit_data_tbl in gold layer.
#               	 Input Arguments for this script are: Phase name (setup), Log file path(/log/file/path)                 										  
#                 2. Executes transform function.
#               	 Input Arguments for this script are: Phase name (transform), Suite name (cablevis-uow-com), suite id (8), 
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_STOCK36_HIT_DATA_TBL}" \
     "${GOLD_STOCK36_HIT_DATA_TBL}" \
...
function fn_module_transform(){
    ...
    pig -useHCatalog -p suite_name=$suite_name \
		...
		-p hive_database_name_gold_ods=${HIVE_DATABASE_NAME_GOLD_ODS} \
		-p hive_database_name_incoming=${HIVE_DATABASE_NAME_INCOMING} \
		-p hive_database_name_gold=${HIVE_DATABASE_NAME_GOLD} \
		-p incoming_hit_data_tbl="${HIVE_TABLE_PREFIX}${INCOMING_HIT_DATA_STOCK36_TBL}" \
		-p gold_cust_services_last_rec="${HIVE_TABLE_PREFIX}${GOLD_CUST_SERVICES_LAST_REC_TBL}" \
		-p gold_stock36_hit_data_tbl="${HIVE_TABLE_PREFIX}${GOLD_STOCK36_HIT_DATA_TBL}" \
