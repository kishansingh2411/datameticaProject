...
#   Purpose:    : Functions to prepare Optimum_Suite table in Incoming layer
#                 1. Executes setup function to create Period table in Incoming Layer.
...
#                                                         Suite Id (9), Source date (2015-12-28),Log file path(/local/file/path)
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_LOOKUP}/${INCOMING_OPTIMUM_SUITE_TBL}" \
     "${INCOMING_OPTIMUM_SUITE_TBL}" \
...
#Preparing Optimum_suite table at incoming layer
function fn_module_prepare(){
  ...
  fn_copy_from_local \
     "${SUBJECT_AREA_HOME}/metadata/Optimum_Suite.txt" \
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_LOOKUP}/${INCOMING_OPTIMUM_SUITE_TBL}" \
     "${BOOLEAN_FALSE}" \
     "${BOOLEAN_FALSE}" \
     "${log_file_path}"
