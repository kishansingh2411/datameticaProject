...
#   Purpose:    : Functions to prepare Connection_type table in Incoming layer
#                 1. Executes setup function to create Connection_type table in Incoming Layer.
#                    Input Arguments for this script are: Phase name (setup), Log file path(/log/file/path)  
#                 2. Executes transform function to build Incoming table.
#                    Input Arguments for this script are: Phase name (transform),Suite name (stock-97), 
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_LOOKUP}/${INCOMING_CONNECTION_TYPE_TBL}" \
     "${INCOMING_CONNECTION_TYPE_TBL}" \
...
#Transforming the Connection_type table at Incoming layer
