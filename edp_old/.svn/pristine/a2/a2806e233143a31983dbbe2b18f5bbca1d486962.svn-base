...
#   Usage       : load_work_modules.sh "key_param_id" "table name" 
#   Date        : 04/12/2016
...
then
    job_name="CHN_"${key_param_id}"_WORK_SPLT_CHT_RST"
elif [[ $table_name == "t_cvb_chdv_a_hh_series_summary" ]]
then
    job_name="CHN_"${key_param_id}"_WORK_HH_SRS_SUMM"
elif [[ $table_name == "t_cvb_f_stb_device_state" ]]
then
    job_name="CHN_"${key_param_id}"_WORK_F_DVC_STATE"
elif [[ $table_name == "t_f_household_device" ]]
then
    job_name="CHN_"${key_param_id}"_WORK_F_HH_DEVICE"
else
...
sh ${SUBJECT_AREA_HOME}/work_${table_name}_tbl/bin/module.sh "prepare" "${source_table}" "${log_file_path}" ${load_start_date} ${load_end_date}
exit_code=$?   
