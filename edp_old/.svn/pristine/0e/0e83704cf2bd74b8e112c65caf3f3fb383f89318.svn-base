...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${SMITH_VOW_ESP_OPTIMUM_USAGE_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_SMITH_SUBJECT_AREA_FACT}/${SMITH_VOW_ESP_OPTIMUM_USAGE_TBL}" \
...
# Deriving vow esp optimum table at Smith layer
fn_module_transform(){
   ...
   beeline -u $HIVESERVER2_URL -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} -hivevar  source_date=$source_date \
      ...
      -hivevar gold_vow_hit_data_esp_tbl="${HIVE_TABLE_PREFIX}${GOLD_VOW_HIT_DATA_ESP_TBL}" \
      -hivevar location="${DATA_LAYER_DIR_SMITH_SUBJECT_AREA_FACT}/${SMITH_VOW_ESP_OPTIMUM_USAGE_TBL}/dtm_created=${dtm_created}/source_date=${source_date}" \
      -f $hive_script_path >> ${log_file_path} 2>&1;
}

# Adding Partition to vow esp optimum table at Smith layer
function fn_module_prepare(){

  source_date=$1
  log_file_path=$2
  dtm_created=`date +"%Y-%m-%d"`
  location_kom="${DATA_LAYER_DIR_SMITH_SUBJECT_AREA_FACT}/${SMITH_VOW_ESP_OPTIMUM_USAGE_TBL}/dtm_created=${dtm_created}/source_date=${source_date}"
  location_mktg="${DATA_LAYER_DIR_SMITH_SUBJECT_AREA_FACT}/${SMITH_VOW_ESP_OPTIMUM_USAGE_TBL}/dtm_created=${dtm_created}/"
