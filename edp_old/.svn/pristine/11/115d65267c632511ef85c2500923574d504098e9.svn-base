...
INSERT OVERWRITE TABLE ${hivevar:hive_database_name_gold}.${hivevar:gold_rsdvr_requests_delta}
PARTITION (load_date)
SELECT
	...
	LAST_UPDATED_IND,
	CREATE_TIME_IND,
	case when WORK_RSDVR_REQUESTS_DEDUP.CHANGEDFLAG='TRUE' then WORK_RSDVR_REQUESTS_DEDUP.OP_TYPE_U else ( case when WORK_RSDVR_REQUESTS_DEDUP.NEWFLAG='TRUE' then WORK_RSDVR_REQUESTS_DEDUP.OP_TYPE_I end) end as OP_TYPE,
	DTM_SYSDATE as DTM_CREATED,
	RSDVR_REQUESTS_DELTA_SEQ as RSDVR_REQUESTS_DELTA_SEQ,
	DATE_FORMAT(LOAD_DATE,'yyyy-MM-dd') as LOAD_DATE
FROM
	${hivevar:hive_database_name_work}.${hivevar:work_rsdvr_requests_dedup_tbl}
WHERE
