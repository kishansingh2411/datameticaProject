...
#                 1. Executes setup function to prepare work_rsdvr_schedule_delete_dedup_tbl table in gold layer.
#               		Input Arguments for this script are: Phase name (setup), Log file path(/log/file/path)                 										  
#                 2. Executes transform function to prepare work_rsdvr_schedule_delete_dedup_tbl table in work layer.
...
function fn_module_setup(){
  ...
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${WORK_RSDVR_SCHEDULE_DELETE_DEDUP_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_SCHEDULE_DELETE_DEDUP_TBL}" \
     "${WORK_RSDVR_SCHEDULE_DELETE_DEDUP_TBL}" \
...
beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      ...
      -hivevar work_rsdvr_schedule_delete_dedup_tbl=${WORK_RSDVR_SCHEDULE_DELETE_DEDUP_TBL} \
