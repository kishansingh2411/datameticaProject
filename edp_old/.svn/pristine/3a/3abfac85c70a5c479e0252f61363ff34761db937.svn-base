...
#   Department: AM
#
#   Program name: load_500601_archive_data_redshift_to_s3.ksh
#   Program type: Unix K Shell script
#   Purpose:    : The purpose of this script is to unload a fact table to s3
#   Author(s)   : DataMetica Team
#   Usage       : ksh load_500601_archive_data_redshift_to_s3.ksh $job_id $key_param
#   Date        : 07/13/2016
#   Log File    : /app-am/logs/edp_offload/load_500601_archive_data_redshift_to_s3.log.*
#   SQL File    :
#   Error File  :
#   Dependency  :
#   Disclaimer  :
...
# Variable assignment
export PROCESS_NAME=load_500601_archive_data_redshift_to_s3.ksh
export FW_LOG_PATH=${AUTODIR}/logs/edp_offload
export COMMON_SCRIPT_PATH=${AUTODIR}/common/bin

#Logfile
LOGFILE_NAME=load_500601_archive_data_redshift_to_s3_${job_id}.log
...
case ${table_name} in
   *_f_*)
      ...
      param_dtm_start=(`psql "host=$REDSHFT_HOST port=$REDSHFT_PORT dbname=$REDSHFT_DATABASE \
         user=$REDSHFT_USER password=$REDSHFT_PASS" -F --no-allign -q -A -t -c \
         "SELECT PARAM_DTM_START FROM CVBMGR.T_KEY_PARAMS WHERE KEY_PARAM_ID = ${key_param_id} ;"`)
      status=$?

      if [ -z ${param_dtm_start} ]
      then
         fn_log_error "Error : Failed while pulling param_dtm_start from key param table. Exiting the program..." "${log_file_path}"
         echo "Error in $0 while pulling param_dtm_start for job_id ${job_id} from key param." | mailx -s "Error in ${PROCESS_NAME} for ${job_id}" $EMAIL_ERROR_LIST
         mv -f ${log_file_path} ${log_file_path}.${date_fmt}
      exit 1
      fi
      
      fn_log_info "Successfully derived param_dtm_start for job_id ${job_id} from key param table." "${log_file_path}"

      #broadcast_month=`echo ${param_dtm_start} | cut -d'-' -f1-2 --output-delimiter=`
      broadcast_month=`date --date=$param_dtm_start '+%Y%m'`

      
      fn_log_info "Starting the DTU for fact table at `date +"%Y-%m-%d_%H:%M:%S"`" "${log_file_path}"

      ## Invoking offload job for fact table ${table_name}
      ${FRAMEWORK_DIR}/aws_master_shell.ksh $job_id "${broadcast_month}" 1>>"${log_file_path}" 2>> "${log_file_path}"
      status=$?

      fn_log_info "DTU finished for fact table at `date +"%Y-%m-%d_%H:%M:%S"`" "${log_file_path}"
   ;;
esac
