...
#                 1. Executes setup function to prepare incoming_t_d_household_device_tbl table in incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DIMENSION}/${INCOMING_T_D_HOUSEHOLD_DEVICE_TBL}" \
     "${INCOMING_T_D_HOUSEHOLD_DEVICE_TBL}" \
...
#Preparing incoming_t_d_household_device_tbl table.
function fn_module_prepare(){   
  table_name=$1
  log_file_path=$2
  source_dir=$3
  
  hadoop fs -test -e ${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DIMENSION}/${INCOMING_T_D_HOUSEHOLD_DEVICE_TBL}
  exit_code=$?
  
  if [ ${exit_code} == 0 ]
  then
     hadoop fs -rm -r "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DIMENSION}/${INCOMING_T_D_HOUSEHOLD_DEVICE_TBL}/*"
  fi
