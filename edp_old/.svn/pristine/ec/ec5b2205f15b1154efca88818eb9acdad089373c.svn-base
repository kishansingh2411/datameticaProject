...
abstract public class CustomTableQuerier implements Comparable<CustomTableQuerier> {
  ...
  public void maybeStartQuery(Connection db) throws SQLException {
    if (resultSet == null) {
      log.info("In method maybeStartQuery.....");	
      log.info("preparing query statement");
      stmt = getOrCreatePreparedStatement(db);
      log.info("done preparing query statement");
      resultSet = executeQuery();
      log.info("In method maybeStartQuery.....got name [ "+name);
      log.info("In method maybeStartQuery.....got resultSet.getMetaData() [ "+resultSet.getMetaData());
