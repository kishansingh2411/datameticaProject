...
function fn_module_prepare(){
  ...
  target_dir_path_temp="${DATA_LAYER_DIR_WORK_SUBJECT_AREA_DATA}/${WORK_OV_TELEPHONE_NUMBER_TBL}" 
  ...
  then
    ...
    ## create query to import data 
     ...
     query="select *  FROM "${ORACLE_DATABASE_OVCDR}.${source_table}" WHERE DTM_LAST_MODIFIED >= TO_DATE('${dtm_last_updated}','yyyy-mm-dd HH24:MI:SS') AND DTM_LAST_MODIFIED < TO_DATE('${current_date}','yyyy-mm-dd HH24:MI:SS') AND \$CONDITIONS"
