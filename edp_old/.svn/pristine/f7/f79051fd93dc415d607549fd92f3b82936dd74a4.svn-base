...
#                 1. Executes setup function to prepare directory structure for the rptctrs table											                                						  
#                    and create incoming_rptctrs_tbl in incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_RPTCTRS_TBL}" \
     "${INCOMING_RPTCTRS_TBL}" \
...
#Preparing incoming EDP_RPTCTRS_TBL table.
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar source_table="${WORK_RPTCTRS_TBL}" \
   -hivevar target_table="${INCOMING_RPTCTRS_TBL}" \
