...
#   Purpose:    : Module script to calculate gold aggregate visit count
#                 1. Executes setup function to prepare directory structure for the table											                                						  
#                    and create gold_aggregate_visit_counts_tbl in gold layer.
...
#   Usage       : setup - 
#						sh module.sh "setup" "$log_file_path" 
#				: Transform - 
...
#Create module data directory if not exists.
  fn_module_create_hive_table_data_layer_directory_structure \
    ...
    "${SMITH_AGGREGATE_VISIT_COUNTS_TBL}" \
    "${BOOLEAN_FALSE}" \
    "${log_file_path}"
 ...
 # Create hive table
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_GOLD_SUBJECT_AREA}/${GOLD_AGGREGATE_VISIT_COUNTS_TBL}" \
     "${GOLD_AGGREGATE_VISIT_COUNTS_TBL}" \
 ...
 # Create hive table
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_SMITH_SUBJECT_AREA_AGGREGATE}/${SMITH_AGGREGATE_VISIT_COUNTS_TBL}" \
     "${SMITH_AGGREGATE_VISIT_COUNTS_TBL}" \
     "${MODULE_HOME}/schema/module.hql" \
     "${log_file_path}" \
     "${HIVESERVER2_URL}" \
...
# Deriving aggregate visit count table at Gold layer
function fn_module_transform(){
   ...
   fn_log_info "checking suite name" "${log_file_path}"
   ...
   then
      ...
      fn_log_error "Got invalid suite_name $suite_name while transform phase of $frequency visit counts aggregate"  \
         ${log_file_path}
      return -1
   fi
   ...
   fn_calculate_aggregates \
      ...
      "${HIVE_TABLE_PREFIX}${GOLD_AGGREGATE_VISIT_COUNTS_TBL}" \
      "${HIVE_TABLE_PREFIX}${SMITH_AGGREGATE_VISIT_COUNTS_TBL}" \
      "$hive_script_path" \
      "${log_file_path}" \
      "$HIVESERVER2_URL" \
      "${HIVE_USENAME}" \
      ...
      then
         fn_log_error "Failed to calculate $frequency Visit Counts aggregates for gold layer, Quitting the process" "${log_file_path}"
	  	 return $exit_code
      fi
}
