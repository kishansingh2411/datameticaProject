...
#                    and create a_dom_nsn_in_usage in work layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_A_DOM_NSN_IN_USAGE_TBL}" \
     "${WORK_A_DOM_NSN_IN_USAGE_TBL}" \
...
# Deriving work_a_dom_nsn_in_usage_tbl table at work layer
...
function fn_module_import(){
  ...
  fn_hadoop_delete_directory_if_exists "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_A_DOM_NSN_IN_USAGE_TBL}/*" \
     "${BOOLEAN_FALSE}" \
     "${log_file_path}"
...
function fn_module_prepare(){
  ...
  if [ ${exit_code} -ne ${EXIT_CODE_SUCCESS} ]
  then
      fn_log_error "Failed to delete directory "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_A_DOM_NSN_IN_USAGE_TBL}/*" " "${log_file_path}"
	  return $exit_code
  fi
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar work_table="${WORK_A_DOM_NSN_IN_USAGE_TBL}" \
   -hivevar incoming_table="${INCOMING_STG_DOM_NSN_IN_USAGE_TBL}" \
   -hivevar gold_table="${GOLD_A_DOM_NSN_IN_USAGE_TBL}" \
   -hivevar work_chc_table="${WORK_CHC_DOM_IN_TBL}" \
...
hql_file="${MODULE_HOME}/hive/module-transform.hql"
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar table_prefix="${HIVE_TABLE_PREFIX}" \
   -hivevar work_table="${WORK_A_DOM_NSN_IN_USAGE_TBL}" \
   -hivevar gold_table="${GOLD_A_DOM_NSN_OUT_USAGE_TBL}" \
   -hivevar work_chc_table="${WORK_CHC_DOM_IN_TBL}" \
   -hivevar file_name="${file_name}" \
   -hivevar ${UNIQUE_PROPERTY_PREFIX}.subject.area.name="${SUBJECT_AREA_NAME}" \
   -hivevar ${UNIQUE_PROPERTY_PREFIX}.project.name="${PROJECT_NAME}" \
