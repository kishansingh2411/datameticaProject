...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_REQUESTS_TMP_TBL}" \
     "${WORK_RSDVR_REQUESTS_TMP_TBL}" \
...
function fn_module_transform(){
	...
	dtm_start=$1
	dtm_end=$2
	source_date=$dtm_start
	log_file_path=$3
  hive_script_path="${MODULE_HOME}/hive/module-transform.hql"

  target_dir="${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_REQUESTS_TMP_TBL}/*"
...
beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      ...
      -hivevar incoming_rsdvr_requests_tbl=${INCOMING_RSDVR_REQUESTS_TBL} \
