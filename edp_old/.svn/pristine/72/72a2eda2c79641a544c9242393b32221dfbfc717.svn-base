...
#                 1. Executes setup function to prepare incoming_rsdvr_schedule_tbl table in incoming layer.
#               		Input Arguments for this script are: Phase name (setup), Log file path(/log/file/path)                 										  
#   
#				  2. Executes transform function to prepare work_rsdvr_schedule_tmp table in work layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_SCHEDULE_TMP_TBL}" \
     "${WORK_RSDVR_SCHEDULE_TMP_TBL}" \
...
#Preparing work_rsdvr_schedule_tmp table.
...
target_dir="${DATA_LAYER_DIR_WORK_SUBJECT_AREA}/${WORK_RSDVR_SCHEDULE_TMP_TBL}/*"
...
beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
      ...
      -hivevar work_rsdvr_schedule_tmp_tbl=${WORK_RSDVR_SCHEDULE_TMP_TBL} \
      -hivevar hive_database_name_incoming=${HIVE_DATABASE_NAME_INCOMING} \
      -hivevar incoming_rsdvr_schedule_tbl=${INCOMING_RSDVR_SCHEDULE_TBL} \
