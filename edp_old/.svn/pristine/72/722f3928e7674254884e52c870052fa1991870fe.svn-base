...
#                    and create incoming_ratepricearea_tbl in incoming layer.
...
function fn_module_setup(){
  ...
  fn_create_hive_table \
     ...
     "${DATA_LAYER_DIR_INCOMING_SUBJECT_AREA_DATA}/${INCOMING_RATEPRICEAREA_TBL}" \
     "${INCOMING_RATEPRICEAREA_TBL}" \
...
#Preparing incoming ratepricearea table.
function fn_module_prepare(){
  ...
  beeline -u ${HIVESERVER2_URL} -n ${HIVE_USENAME} -p ${HIVE_PASSWORD} \
   ...
   -hivevar source_table="${WORK_RATEPRICEAREA_TBL}" \
   -hivevar target_table="${INCOMING_RATEPRICEAREA_TBL}" \
